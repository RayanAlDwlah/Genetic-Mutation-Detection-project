...
VALIDATION_PATH = REPO_ROOT / "data" / "processed" / "validation_report.json"
...
METADATA_OUTPUT = REPO_ROOT / "data" / "processed" / "feature_analysis_metadata.json"
...
def step1_drop_flagged_columns(df: pd.DataFrame, validation_report: dict[str, Any]) -> tuple[pd.DataFrame, list[str], list[str]]:
    col_quality = validation_report.get("column_quality", {})
    all_nan = col_quality.get("all_nan_columns", []) or []
    zero_var = col_quality.get("zero_variance_columns", []) or []

    drop_nan = [c for c in all_nan if c in df.columns]
    drop_zero = [c for c in zero_var if c in df.columns and c not in drop_nan]

    to_drop = drop_nan + drop_zero
    if to_drop:
        df = df.drop(columns=to_drop)

    print("STEP 1 — Columns dropped from validation flags")
    print(f"- 100% NaN columns dropped: {drop_nan if drop_nan else 'none'}")
    print(f"- zero-variance columns dropped: {drop_zero if drop_zero else 'none'}")

    return df, drop_nan, drop_zero


def choose_drop_feature(
    col_a: str,
    col_b: str,
    corr_pair: dict[str, Any],
    ...
    # Tie-breaker from literature priority for specific families.
    ...
    suggested = corr_pair.get("suggest_drop")
    if isinstance(suggested, str) and suggested in {col_a, col_b}:
        keep = col_b if suggested == col_a else col_a
        return suggested, keep, "validation suggestion"
...
def step2_drop_correlated_columns(df: pd.DataFrame, validation_report: dict[str, Any]) -> tuple[pd.DataFrame, list[dict[str, Any]]]:
    corr_pairs = validation_report.get("feature_correlation", {}).get("high_correlation_pairs_abs_gt_0_95", []) or []
    ...
    for pair in corr_pairs:
        ...
        drop_col, keep_col, reason = choose_drop_feature(col_a, col_b, pair, missing_pct)
...
def make_balanced_dataset(df: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for col in feature_cols_after_drop:
        ...
        if pd.api.types.is_numeric_dtype(s):
            ...
            impute_stats[col] = {"type": "numeric", "fill_value": float(fill_value)}
        else:
            mode = s.mode(dropna=True)
            fill_value = mode.iloc[0] if not mode.empty else "UNKNOWN"
            balanced[col] = s.fillna(fill_value)
            impute_stats[col] = {"type": "categorical", "fill_value": str(fill_value)}
...
def main() -> None:
    ...
    # Step 1
    df, dropped_nan_cols, dropped_zero_cols = step1_drop_flagged_columns(df, validation)

    # Step 2
    df, dropped_corr_records = step2_drop_correlated_columns(df, validation)
    ...
    # Step 4 summaries
    strict_summary = version_summary("strict", strict_df)
    balanced_summary = version_summary(
        "balanced",
        balanced_df,
    ...
    metadata = {
        ...
        "columns_dropped_nan": dropped_nan_cols,
        "columns_dropped_zero_variance": dropped_zero_cols,
        "columns_dropped_correlation": dropped_corr_records,
        "strict_version": {
            "rows": strict_summary["rows"],
            "columns": strict_summary["columns"],
            "pathogenic": strict_summary["pathogenic"],
            "benign": strict_summary["benign"],
            "missing_policy": strict_info["missing_policy"],
            "max_missing_pct_after_processing": strict_summary["max_missing_pct_after_processing"],
        },
        "balanced_version": {
            "rows": balanced_summary["rows"],
            "columns": balanced_summary["columns"],
            "pathogenic": balanced_summary["pathogenic"],
            "benign": balanced_summary["benign"],
            "imputation_method": balanced_info["imputation_method"],
            "missing_policy": balanced_info["missing_policy"],
            "max_missing_pct_pre_imputation": balanced_info["max_missing_pct_pre_imputation"],
        },
        "final_feature_list": get_feature_columns(balanced_df),
    }
