...
SHAP_DIR = REPO_ROOT / "results" / "shap"
FIGURES_DIR = REPO_ROOT / "figures"
FIGURES_DIR.mkdir(exist_ok=True)

# Default file paths — adjust to your actual paths
PHASE1_PARQUET = SHAP_DIR / "phase1_shap.parquet"
PHASE2_PARQUET = SHAP_DIR / "phase2_shap.parquet"
...
def load_shap_summary(parquet_path: Path, top_n: int = 15) -> pd.DataFrame:
    """
    Load a SHAP summary parquet and return the top-N features by mean |SHAP|.
    ...
    if not parquet_path.exists():
        raise FileNotFoundError(f"SHAP parquet not found: {parquet_path}")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=f"Phase-1 SHAP parquet (default: {PHASE1_PARQUET})",
    )
    parser.add_argument(
        "--phase2-path",
        type=Path,
        default=PHASE2_PARQUET,
        help=f"Phase-2.1 SHAP parquet (default: {PHASE2_PARQUET})",
    )
    parser.add_argument(
        "--top-n",
        type=int,
