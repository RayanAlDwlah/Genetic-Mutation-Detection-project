...
def fig_leakage_journey() -> None:
    ...
    for xi, v in zip(x, roc):
        ax.annotate(f"{v:.3f}", (xi, v), textcoords="offset points",
                    xytext=(0, 9), ha="center", color="#1f77b4", fontsize=9)
    for xi, v in zip(x, pr):
        ax.annotate(f"{v:.3f}", (xi, v), textcoords="offset points",
                    xytext=(0, -15), ha="center", color="#d62728", fontsize=9)
    ax.set_xticks(x)
    ax.set_xticklabels(
        ["1. Pre-audit\n(0.955)", "2. Missense\nfilter", "3. Feature\nhygiene",
         "4. Paralog-aware\nsplit", "5. Optuna\ntuning"],
    ...
    ax.set_title("The Leakage Hunt — five audit stages, one honest baseline",
                 fontsize=12, fontweight="bold")
    ax.set_ylim(0.78, 0.98)
    ax.grid(axis="y", alpha=0.25)
    ax.legend(loc="lower left", frameon=False)
    ax.annotate("−13.6 PR-AUC pts from\nfixing non-missense\ncontamination",
                xy=(1, 0.819), xytext=(1.4, 0.86),
...
def fig_reliability() -> None:
    ...
    for ax, which in zip(axes, ["val", "test"]):
        ...
        ax.plot(raw["mean_predicted_prob"], raw["fraction_positive"],
                marker="o", lw=1.6, color="#d62728",
                label=f"raw  (ECE={summary.loc[f'{which}_raw','ECE']:.3f})")
        ax.plot(cal["mean_predicted_prob"], cal["fraction_positive"],
                marker="s", lw=1.6, color="#2ca02c",
        ...
        ax.grid(alpha=0.25)
    fig.suptitle("Probability calibration — isotonic brings ECE from 0.07 → 0.015",
...
def fig_pr_roc() -> None:
    ...
    ci = pd.read_csv(MET / "xgboost_bootstrap_ci.csv").set_index("metric_set").loc["test_calibrated"]
