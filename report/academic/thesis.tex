...
LOEUF, mis\_z, oe\_mis\_upper, lof\_z) raises the point
estimate to $0.573$. The un-paired bootstrap CIs of
$[0.383, 0.583]$ (pre) and $[0.471, 0.672]$ (post) overlap;
a paired bootstrap on the same 201 variants is the correct
instrument and is recorded as deficiency D8 of
Section~\ref{sec:conc-deficiencies}, pending persistence
of the pre-constraint per-variant predictions. We therefore
interpret the improvement as \emph{suggestive} of genuine
signal from gene-level priors rather than conclusive, and
frame it as a partial closure of the generalisation gap
that motivates Phase-2 training-time integration. A zero-shot
ESM-2 log-likelihood-ratio provides a small, statistically tested
rank-fusion gain on both the test split and the \textsc{denovo-db}
family-holdout, indicating non-redundant signal between tabular and
protein-language-model features. The post-constraint improvement on
the 201-variant family-holdout slice is a point estimate whose
confidence interval still includes chance and is reported as
suggestive rather than conclusive (see Section~\ref{sec:conc-limits}).
Full training-time integration of the protein-language-model and of
AlphaFold2 structural features is documented as future work
...
likelihood-ratio signal is not redundant with the
tabular feature set on two independent evaluation splits
(test and \textsc{denovo-db} family-holdout), with
statistically tested rank-fusion gains reported in
...
After training, \texttt{num\_\_esm2\_llr} appears at rank~4 in the
mean-$|$SHAP$|$ ranking on the stratified $2{,}000$-variant test
sample (\texttt{results/metrics/shap\_ranking\_phase21.csv}).
The companion \texttt{num\_\_is\_imputed\_esm2\_llr} flag appears
at rank~1, which is initially surprising but consistent with the
biology: variants whose isoform context defeated the ESM-2
...
The Optuna-tuned Phase-2.1 model achieves test PR-AUC $=0.8698$
(versus frozen-hp $0.8710$); the absolute difference is $0.0012$,
well below our $0.005$ pre-registered threshold for treating
the two as substantively distinct. Optuna and frozen-hp agree
...
Beyond the seven deficiencies listed above, four
...
ESM-2 integration) is in active development at the time of
submission and is expected to complete within 4--6 weeks of
this thesis's defence, contingent on Colab T4 availability.
Phase~2.2 (AlphaFold2 structural features) is scoped at $3$
months given that the pLDDT and DSSP extraction code is
already wired in. Phase~2.3 (ProteinGym external
