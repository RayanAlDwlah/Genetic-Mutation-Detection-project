...
\begin{tikzpicture}[
  font=\sffamily\footnotesize,
  bar/.style={draw, rounded corners=1.5pt, minimum height=0.38cm, inner sep=0.5pt},
  s1/.style={bar, fill=blue!25},
  s2/.style={bar, fill=orange!35},
  mile/.style={diamond, draw, fill=red!50, minimum size=0.28cm, inner sep=0pt},
  lab/.style={font=\sffamily\scriptsize, anchor=east},
  month/.style={font=\sffamily\scriptsize, anchor=south},
  x=0.7cm, y=0.45cm,
]
  % X-axis (months)
  \foreach \x/\m in {0/Sep, 1/Oct, 2/Nov, 3/Dec, 4/Jan, 5/Feb, 6/Mar, 7/Apr, 8/May} {
    \draw[gray!40] (\x, 0) -- (\x, -10);
    \node[month] at (\x+0.5, 0.05) {\m};
  }
  \draw[gray!70, thick] (0,0) -- (9,0);

  % Term separators
  \draw[red!60, thick, dashed] (4, 0.5) -- (4, -10.5) node[below, font=\sffamily\scriptsize, red!70] {Term 1 $\to$ Term 2};

  % Tasks (row, start, duration, style)
  \node[lab] at (0, -1) {Literature review};           \node[s1, minimum width=2.8*0.7cm*1cm, anchor=west] at (0.05, -1) {};
  \node[lab] at (0, -2) {Data ingestion};              \node[s1, minimum width=2.2*0.7cm*1cm, anchor=west] at (0.8, -2) {};
  \node[lab] at (0, -3) {XGBoost baseline};            \node[s1, minimum width=2.0*0.7cm*1cm, anchor=west] at (1.8, -3) {};
  \node[lab] at (0, -4) {Leakage audit + paralog fix}; \node[s2, minimum width=1.4*0.7cm*1cm, anchor=west] at (4.0, -4) {};
  \node[lab] at (0, -5) {Optuna HPO + calibration};    \node[s2, minimum width=1.0*0.7cm*1cm, anchor=west] at (4.6, -5) {};
  \node[lab] at (0, -6) {SHAP + baselines};            \node[s2, minimum width=0.9*0.7cm*1cm, anchor=west] at (5.2, -6) {};
  \node[lab] at (0, -7) {denovo-db external validation};\node[s2, minimum width=0.8*0.7cm*1cm, anchor=west] at (5.8, -7) {};
  \node[lab] at (0, -8) {ESM-2 zero-shot (Colab)};     \node[s2, minimum width=1.0*0.7cm*1cm, anchor=west] at (6.1, -8) {};
  \node[lab] at (0, -9) {LaTeX thesis + defense};      \node[s2, minimum width=1.5*0.7cm*1cm, anchor=west] at (6.3, -9) {};
  \node[lab] at (0, -10) {Plagiarism check + final};   \node[s2, minimum width=0.4*0.7cm*1cm, anchor=west] at (7.8, -10) {};

  % Milestones
  \node[mile] at (2.0, -3) {}; \node[anchor=south west, font=\sffamily\tiny, gray!60] at (2.0, -2.7) {Baseline ROC 0.955};
  \node[mile] at (4.8, -4) {}; \node[anchor=south west, font=\sffamily\tiny, gray!60] at (4.8, -3.7) {Paralog-disjoint split};
  \node[mile] at (6.0, -7) {}; \node[anchor=south west, font=\sffamily\tiny, gray!60] at (6.0, -6.7) {denovo-db locked};
  \node[mile] at (7.1, -8) {}; \node[anchor=south west, font=\sffamily\tiny, gray!60] at (7.1, -7.7) {ESM-2 val+test};
  \node[mile] at (8.2, -10) {}; \node[anchor=south east, font=\sffamily\tiny, gray!60] at (8.2, -9.7) {Final defense};

  % Legend
  \node[s1, minimum width=0.35cm] at (4.6, -11.3) {}; \node[anchor=west, font=\sffamily\scriptsize] at (4.85, -11.3) {Term 1};
  \node[s2, minimum width=0.35cm] at (6.1, -11.3) {}; \node[anchor=west, font=\sffamily\scriptsize] at (6.35, -11.3) {Term 2};
  \node[mile] at (7.6, -11.3) {};                     \node[anchor=west, font=\sffamily\scriptsize] at (7.85, -11.3) {Milestone};
\end{tikzpicture}
\caption[Two-semester Gantt chart]{\textbf{Two-semester Gantt
chart.} Term 1 covered literature review, data ingestion,
and a first XGBoost baseline. Term 2 covered the leakage
audit, hyperparameter search, calibration, SHAP analysis,
external validation, ESM-2 scoring, and the academic write-
up. The five red diamonds mark unambiguous quantitative
milestones tracked in \texttt{docs/CHANGELOG.md}.}
\label{fig:gantt}
\end{figure}
...
\texttt{facebook/esm2\_t12\_35M\_UR50D}, has
...
\midrule
Unit & \texttt{tests/unit/test\_metrics.py} & \texttt{test\_bootstrap\_ci\_variance\_shrinks\_with\_n} & Metric math correctness under random seeds. \\
Unit & \texttt{tests/unit/test\_paralog.py} & \texttt{test\_keratin\_family\_regex} & Family grouper is total and deterministic. \\
Property & \texttt{tests/property/test\_calibration.py} & \texttt{test\_isotonic\_is\_monotone} & Calibrator respects order for any fitted mapping. \\
Integration & \texttt{tests/integration/test\_leakage\_gate.py} & \texttt{test\_no\_banned\_features} & All four invariants of Section~\ref{sec:fw-part2}. \\
Integration & \texttt{tests/integration/test\_dbnsfp\_cache.py} & \texttt{test\_cache\_miss\_falls\_back\_to\_vep} & Fallback path functions end-to-end. \\
Scientific & \texttt{tests/integration/test\_reproduce\_headline.py} & \texttt{test\_test\_split\_rocauc\_matches} & Headline number matches to six decimals. \\
Scientific & \texttt{scripts/evaluate\_external.py} & driver script & denovo-db numbers reproduce from clone. \\
CI gate & \texttt{.github/workflows/test.yml} & GH Actions & All levels must pass before merge. \\
\bottomrule
\end{tabularx}
