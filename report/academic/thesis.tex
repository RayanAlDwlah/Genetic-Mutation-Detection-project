...
own-coverage numbers our XGBoost reaches PR-AUC $0.838$; SIFT
$0.620$, PolyPhen-2 $0.728$, and AlphaMissense $0.890$. A strict
...
submission time. AlphaMissense was moreover calibrated on \textsc{ClinVar}
at release and therefore carries contamination risk of its own.
...
\item \textbf{A fully reproducible code base} -- 157 tests,
...
Chapter~\ref{ch:testing} documents the five-level testing
strategy, coverage evidence, and the continuous-integration
gate that enforces reproducibility.
...
for future modelling work. The stratified error
analysis below (Section~\ref{sec:res-errors}) shows
that these confident errors cluster at low
conservation, consistent with the ablation result that
conservation carries most of the signal.
...
Chapter~\ref{ch:related}, our key distinguishing features
are: a paralog-aware test split, explicit bootstrap CIs
on every metric, a re-scoring of published baselines on
own-coverage subsets (matched-coverage is deferred to
D7), a pre/post attribution of gene-level priors on
external generalisation, and full reproducibility
enforced by CI. On own-coverage numbers,
AlphaMissense reports PR-AUC $0.890$ on its $86\%$
coverage subset, our XGBoost reports $0.838$ on $100\%$;
...
paired test is the first item of the revision roadmap.
...
per model), XGBoost reaches PR-AUC $0.838$ against SIFT's
$0.620$ and PolyPhen-2's $0.728$. AlphaMissense reaches
