...
and a Phase-2.1 training-time integration of the same LLR as a numeric
...
ceiling at the 35\,M-parameter scale and motivating Phase-2.1b (a
larger ESM-2 checkpoint) and Phase-2.2 (AlphaFold2 structural features)
...
family-holdout), \emph{and} a Phase-2.1 training-time
...
implementation, the Phase-1 baseline results, the like-for-like
comparison tables, and the Phase-2.1 training-time integration
...
\textbf{Pros.} Architecturally closest in spirit to our Phase-2 plan
...
large-scale DMS benchmark with 217 assays that our Phase-2 roadmap
...
revision item and is the first entry on the Phase-2 list of
...
The curve has three implications for the Phase-2 roadmap.
First, adding \emph{more labelled variants} of the same kind is
unlikely to raise the baseline materially --- test ROC-AUC has
plateaued. Second, this means the Phase-2 emphasis should be on new
...
constraint signal when conservation is weak. Phase-2.2
(AlphaFold2 structural features, Section~\ref{sec:conc-future})
and Phase-2.1 (ESM-2 LLR as a training feature) are both
directly motivated by this failure mode.
...
reference in both regimes; the Phase-2.1 full-training-set
...
motivate the Phase-2.1 decision to retrain the tabular
...
baselines were re-scored under our methodology; our two rows are
\emph{Phase-1} (XGBoost only) and \emph{Phase-2.1} (XGBoost + ESM-2
LLR as a training feature, see Section~\ref{sec:res-phase21}). PR-AUC
columns use each method's own coverage subset (SIFT $96\%$, PP2
$93\%$, AM $86\%$, ours $100\%$) per Table~\ref{tab:baselines}; a
strict matched-coverage comparison is deferred to deficiency D7.
$^{*}$~AlphaMissense was calibrated against \textsc{ClinVar} at
release time. $^{\dagger}$~REVEL is excluded from the feature set
in our pipeline because it stacks 13 base learners, several of
which were trained on \textsc{ClinVar}-overlapping populations,
producing compound contamination
(Section~\ref{sec:rw-revel}).}
...
\midrule
Phase-1 (ours)   & 2026 & XGBoost on 39 audited features & \textbf{yes} & \textbf{yes (3 src)} & \textbf{yes (denovo-db)} & 0.827 \\
\rowcolor{green!8}
\textbf{Phase-2.1 (ours)} & \textbf{2026} & \textbf{Phase-1 $+$ ESM-2 LLR feature} &
...
\emph{First,} on the two classical tools (SIFT, PolyPhen-2),
the leakage-audited Phase-2.1 baseline beats them by $+0.245$
and $+0.137$ PR-AUC respectively. These are large,
methodologically defensible margins --- because SIFT and
PolyPhen-2 pre-date our paralog-splitting infrastructure, no
paralog-leakage contamination inflates their score.

\emph{Second,} against AlphaMissense, the Phase-2.1 own-coverage
gap is $-0.025$ PR-AUC (down from Phase-1's $-0.063$). The
remaining gap is structurally explicable:
...
interpretation is: AlphaMissense is stronger as a
...
empty. Whether the $-0.025$ \textsc{ClinVar} gap closes, widens,
or inverts on \textsc{denovo-db} is an obvious Phase-2 experiment.

\emph{Third,} on the in-distribution test split, the marginal
contribution of training-time PLM integration is statistically
significant: Phase-2.1 vs Phase-1 paired bootstrap gives
$\Delta\text{PR-AUC}=+0.0313$ ($p<10^{-4}$, $n=28{,}098$,
...
\emph{Fourth,} the thesis's unique contribution against every
row above is not raw benchmark performance --- it is the
methodological discipline bundle of \textbf{paralog-disjoint
splitting}, \textbf{three-source leakage audit},
\textbf{external validation on denovo-db}, \textbf{paired-bootstrap
inference}, \textbf{Murphy-decomposed calibration}, and
\textbf{Docker-pinned reproducibility}. None of the eight
reviewed methods reports all six in a single evaluation; the
``Paralog guard / Leakage audit / External val.'' columns of the
table above quantify this gap concretely.

On own-coverage numbers, AlphaMissense reports PR-AUC $0.890$ on
its $86\%$ coverage subset; our Phase-2.1 model reports
...
\textbf{Note on ESM-2 Colab progress.} The Phase-2.1
...
(\texttt{data/intermediate/esm2/}). The Phase-2.1
training-time integration described in Section~\ref{sec:res-phase21}
consumed these parquets directly.

\section{Phase 2.1: ESM-2 LLR as a Training Feature}
\label{sec:res-phase21}

This section reports the Phase-2.1 integration of the zero-shot
ESM-2 LLR as an additive numeric feature in the XGBoost training
matrix, replacing the post-hoc rank-fusion approach of
Section~\ref{sec:res-esm2}. The Phase-1 baseline of
Section~\ref{sec:res-headline} is \emph{frozen}; every Phase-2.1
result reported below is a NEW row in a comparison table, not
an overwrite. All artifacts are emitted under a
\texttt{phase21}/\texttt{\_phase21} namespace
(\texttt{results/checkpoints/xgboost\_phase21\_esm2.ubj},
\texttt{results/metrics/phase21/}) so the Phase-1 numbers are
...
\textbf{Hyperparameters are frozen at the Phase-1 Optuna optimum.}
...
Phase-1 \texttt{xgboost\_best\_params.csv} and
...
three XGBoost models trained on the Phase-2.1 splits with the
frozen hyperparameters, varying only the feature set.
\texttt{full} is the canonical Phase-2.1 model; \texttt{no\_esm2}
drops the two ESM-2 columns and is the apples-to-apples Phase-1
...
\begin{table}[H]
\centering
\caption[Phase-2.1 ablation]{\textbf{Phase-2.1 ablation on the
paralog-disjoint test split, calibrated probabilities,
$1{,}000$-replicate bootstrap CIs.} Phase-1 numbers are quoted
verbatim from Table~\ref{tab:headline} for reference. The
\texttt{full} model is the headline Phase-2.1 result; the paired
...
\midrule
Phase-1 baseline (frozen)        & 78 & 0.9376 $[0.9346, 0.9404]$ & 0.8273 $[0.8185, 0.8352]$ \\
Phase-2.1 \texttt{no\_esm2}      & 78 & 0.9386 $[0.9355, 0.9416]$ & 0.8396 $[0.8311, 0.8474]$ \\
Phase-2.1 \texttt{esm2\_only}    &  2 & 0.7388 $[0.7314, 0.7458]$ & 0.6099 $[0.5987, 0.6205]$ \\
\rowcolor{green!8}
\textbf{Phase-2.1 \texttt{full}} &\textbf{80} &\textbf{0.9476 $[0.9448, 0.9503]$} &\textbf{0.8647 $[0.8580, 0.8714]$} \\
\bottomrule
\end{tabular}
\end{table}
...
\caption[Phase-1 vs Phase-2.1 SHAP comparison]{\textbf{Top-15
features by mean-$|$SHAP$|$, Phase-1 vs Phase-2.1.} The
ESM-2 imputation flag and LLR enter the Phase-2.1 ranking at
...
The Phase-2.1 model was scored on the same $642$ \textsc{denovo-db}
variants (and the $201$ family-holdout slice) as Phase-1.
Table~\ref{tab:phase21-denovo} reports the comparison.

\begin{table}[H]
\centering
\caption[Phase-1 vs Phase-2.1 on denovo-db]{\textbf{Phase-1
versus Phase-2.1 on \textsc{denovo-db}.} Calibrated
probabilities; paired bootstrap $\Delta$ uses the same variants
in both columns, $1{,}000$ replicates, \texttt{seed=42},
one-sided test of $H_0\!:\,\Delta\le 0$. Phase-1 numbers come
from \texttt{external\_denovo\_db\_predictions.parquet}
(\textit{cf.}~Section~\ref{sec:res-denovo}); Phase-2.1 numbers
from \texttt{external\_denovo\_db\_predictions\_phase21.parquet}.}
\label{tab:phase21-denovo}
\small
\begin{tabular}{@{} l r l l c @{}}
\toprule
\textbf{Slice / metric} & $n$ & \textbf{Phase-1} & \textbf{Phase-2.1} & \textbf{Paired $\Delta$ (95\% CI), $p$} \\
\midrule
...
denovo-db slice the Phase-2.1 model is statistically
indistinguishable from Phase-1 ($p=0.51$/0.62); on the
...
completeness. The canonical Phase-1 evaluation joined denovo-db
variants against the raw gnomAD-constraint TSV; in this revision
worktree the raw TSV is unavailable, and the Phase-2.1 scorer
...
values for Phase-2.1 even where Phase-1 had real values from the
...
are the \emph{frozen-hp} Phase-2.1 model. To confirm the
hyperparameter choice does not silently dominate the marginal
effect, we re-ran Optuna ($40$ trials, \texttt{seed=42}) on the
Phase-2.1 splits and report the result here only.

The Optuna-tuned Phase-2.1 model achieves test PR-AUC
...
canonical Phase-2.1 baseline and is what every other table in
...
Isotonic refit on the Phase-2.1 validation predictions gives
test ECE $=0.0056$ (versus Phase-1 ECE $=0.0105$), test MCE
$=0.0256$, and Brier loss $=0.0748$ (versus Phase-1 $=0.0826$).
Calibration is therefore strictly tighter on Phase-2.1 across
...
Phase-2.1 ESM-2 integration lifts in-distribution test
...
\item \textbf{OOD ceiling on protein language models at the
  35\,M-parameter scale.} The Phase-2.1 integration of
...
(completed; see Section~\ref{sec:res-phase21}).}
Zero-shot LLR was scored on all training, validation,
and test variants on a Colab T4 GPU; XGBoost was
retrained with \texttt{esm2\_llr} +
\texttt{is\_imputed\_esm2\_llr} as features under frozen
Phase-1 hyperparameters. Internal-test paired bootstrap
gives $\Delta$PR-AUC $=+0.031$ ($p<10^{-4}$); external
\textsc{denovo-db} family-holdout gives
$\Delta$ROC-AUC $=-0.13$ ($p\!=\!0.98$ one-sided), driven
in part by a constraint-coverage confound on unseen
gene families.
\item \textbf{Phase 2.1b -- Larger ESM-2 checkpoint.}
The current 35\,M-parameter
...
Phase 2.1 with \texttt{esm2\_t33\_650M\_UR50D}
...
\paragraph{Indicative timeline.} Phase~2.1 (full-dataset
ESM-2 integration) was completed during the final revision
cycle and is fully reported in Section~\ref{sec:res-phase21}.
Phase~2.1b (upgrade to \texttt{esm2\_t33\_650M\_UR50D}, the
$650$\,M-parameter checkpoint, $19\times$ parameters) is the
immediate next step and is scoped at $4$--$6$ weeks contingent
on Colab T4 availability, motivated by the in-distribution
ceiling and OOD regression documented in
Section~\ref{sec:phase21-limits}. Phase~2.2 (AlphaFold2
structural features) is scoped at $3$ months given that the
pLDDT and DSSP extraction code is already wired in. Phase~2.3 (ProteinGym external
validation) requires implementing a regression-loss
variant and is scoped at $2$ months. Phase~2.4 (hybrid
stacking and VEP plug-in) is a $6$-month follow-on.
Phase~2.5 (deployment hardening) is out of the authors'
current scope and would require either a continuation by a
successor team or SFDA engagement.
