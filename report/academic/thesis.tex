...
the zero-shot ESM-2 LLR alone achieves ROC-AUC of
$0.552$. Rank fusion with the XGBoost calibrated
probability raises the score to ROC-AUC $= 0.588$ -- a
$+0.015$ additive gain under the same rank-fusion
scheme. Taken together, the test-set and external
numbers constitute two independent empirical signals
that the two models carry orthogonal information, which
motivates the Phase-2.1 decision to integrate ESM-2
as a training-time feature on the full corpus rather
than only as a post-hoc fusion partner.
