...
benchmarks. We hypothesise --- and in this work demonstrate on a concrete
pipeline --- that such numbers can be inflated by subtle, removable forms
of data contamination, and we show that external validation on an
independent cohort reveals a substantial generalisation gap. This project
develops and publishes a rigorous, auditable baseline for the missense
classification task.

We assemble a corpus of $283{,}392$ candidate variants from \textsc{ClinVar},
\textsc{gnomAD} (v2.1 and v4), and \textsc{dbNSFP} version 5.3.1a, of which
$195{,}098$ survive a strict missense-only filter and feed the training
pipeline. We then conduct a systematic leakage audit that eliminates
three contamination sources: (i)~non-missense variants that had been silently admitted into
...
our calibrated test-set precision-recall area-under-curve (PR-AUC) from a
misleading $0.955$ to a defensible $0.827$, with a 95\% bootstrap confidence
interval of $[0.819, 0.835]$. The uncalibrated PR-AUC is $0.836$
$[0.827, 0.843]$; all downstream evaluation in this report uses the
isotonic-calibrated probabilities. The model is calibrated end-to-end with
isotonic regression, reducing expected calibration error (ECE) from
$0.054$ to $0.011$, which is below the $0.02$ clinical target.

We re-score three published predictors (SIFT, PolyPhen-2, AlphaMissense)
on our paralog-disjoint test split. Because each baseline covers a
different subset of variants, the reported numbers are on each model's
\emph{own coverage} (SIFT 96\,\%, PolyPhen-2 93\,\%, AlphaMissense
86\,\%, XGBoost 100\,\%) and are therefore not directly comparable. On
own-coverage numbers our XGBoost reaches calibrated PR-AUC $0.827$
(raw $0.836$); SIFT $0.620$, PolyPhen-2 $0.728$, and AlphaMissense
$0.890$. A strict
matched-coverage evaluation restricted to the AlphaMissense-covered
subset, together with a paired DeLong test, is left for a revision
(Deficiency D7) since the required artefacts were not persisted at
submission time. AlphaMissense was moreover calibrated against
\textsc{ClinVar} at release; direct benchmark comparisons on
\textsc{ClinVar}-derived test sets are therefore non-trivial to
interpret.
...
LOEUF, mis\_z, oe\_mis\_upper, lof\_z) raises the point estimate to
$0.573$. We later run a paired bootstrap on the same 201 family-holdout
variants using the recovered pre-constraint checkpoint and report
$\Delta\text{ROC-AUC}=+0.0764$, 95\% CI $[-0.0207, +0.1720]$, one-sided
$p=0.073$ (Section~\ref{sec:res-denovo}); the companion paired PR-AUC
$\Delta=+0.0375$ reaches $p=0.041$. Even under the paired view a
ROC-AUC near $0.57$ on unseen gene families is closer to chance than
to clinical utility, so we frame the result as a \emph{partial closure
of the generalisation gap} rather than as a deployable predictor. A
zero-shot ESM-2 log-likelihood-ratio provides additional, statistically
tested rank-fusion gain on the test split (Section~\ref{sec:res-esm2}),
and a training-time integration of the ESM-2 LLR of the same LLR as a numeric
feature (Section~\ref{sec:res-phase21}) lifts calibrated test PR-AUC
from $0.827$ to $0.865$ ($\Delta=+0.0313$, paired $p<10^{-4}$);
the same feature does \emph{not} transfer to unseen gene families
($\Delta\text{ROC-AUC}=-0.13$ on the family-holdout slice, partly
attributable to a constraint-coverage confound), defining a documented
ceiling at the 35\,M-parameter scale and motivating the 650\,M-parameter ESM-2 extension (a
larger ESM-2 checkpoint) and the AlphaFold2 structural-feature extension
as future work (Section~\ref{sec:conc-future}).

The project's code base is published in full with $157$ passing
tests (1 GPU-gated smoke test skipped), $59\%$ line coverage,
a CI-enforced leakage gate, pinned dependencies, and a Docker
image that reproduces every number in this report byte-for-byte.
...
Hereditary information is encoded in DNA and flows, under the
\emph{central dogma} of molecular biology
\citep{crick1970central}, from DNA to messenger RNA via
transcription and from mRNA to protein via translation.
Figure~\ref{fig:central-dogma} illustrates the pathway alongside
...
A genetic mutation is any permanent alteration in the DNA
template; most are neutral, a small fraction are beneficial, and
a clinically important minority disrupt cellular function
\citep{macarthur2014guidelines}. The single-nucleotide
\emph{missense} variant --- a point mutation that swaps one amino
acid for another --- is the most common coding variant in the
human population and the focus of this project.

\subsection{Classes of Protein-Coding Variation}
\label{sec:intro-variantclasses}

Within the protein-coding genome, a single-base substitution
takes one of four functional forms \citep{alberts2015molecular}:

\begin{description}[style=nextline, leftmargin=2.2em, itemsep=0.25em]
  \item[Synonymous.] Same amino acid (e.g.\ \texttt{GCU}\,$\rightarrow$\,\texttt{GCC}).
  \item[Missense.] Different amino acid (e.g.\ \texttt{GAC} Asp\,$\rightarrow$\,\texttt{AAC} Asn) --- the class studied here.
  \item[Nonsense.] Premature stop codon (e.g.\ \texttt{UGG} Trp\,$\rightarrow$\,\texttt{UGA} stop).
  \item[Frameshift.] Non-triplet indel that scrambles every downstream codon.
\end{description}

Missense variants are of special interest because their clinical
effect is genuinely uncertain at the individual level: unlike
nonsense (usually pathogenic) or synonymous (usually benign)
variants, a missense change can produce anything from a
wild-type-equivalent protein to a catastrophically misfolded one.
...
and how to combine them non-linearly. Figure~\ref{fig:feature-groups}
in Chapter~\ref{ch:framework} shows how each of the five signals
here maps to a concrete feature group in our implementation.
...
\item \textbf{A two-pass ESM-2 integration} --- a
zero-shot proof-of-concept establishing that the
protein-language-model log-likelihood-ratio is not
redundant with the tabular feature set
(Section~\ref{sec:res-esm2}, with statistically tested
rank-fusion gains on test and \textsc{denovo-db}
family-holdout), \emph{and} an additional training-time
integration that lifts calibrated test PR-AUC from
$0.827$ to $0.865$ ($\Delta=+0.0313$, paired bootstrap
$p<10^{-4}$; Section~\ref{sec:res-phase21}). The same
training-time feature does not transfer to unseen gene
families, defining a documented ceiling at the
35\,M-parameter checkpoint scale.
\item \textbf{An auditable methodological package} --- a
CI-enforced leakage gate encoding four invariants
(banned features, missense-only, paralog disjointness,
class-balance drift), a paralog-family grouper regex
covering 16 clinically-relevant paralog clusters, and a
train-only imputation contract for gnomAD constraint
merging --- that can be lifted into future
missense-classification work as a self-contained module.
\item \textbf{Engineering artifacts that make the
methodological package verifiable}: 157 passing tests
(1 GPU-gated skip), 59\% line coverage, a pinned
dependency lock, a Docker image, 10 narrative Jupyter
notebooks, a Streamlit demo, and this LaTeX report ---
supporting byte-for-byte reproduction from a clean
clone.
\end{enumerate}
...
implementation, the XGBoost baseline results, the like-for-like
comparison tables, and the training-time integration of the ESM-2 LLR
of the ESM-2 LLR (Section~\ref{sec:res-phase21}).
Chapter~\ref{ch:testing} describes the five-level testing
strategy, coverage evidence, continuous-integration gates,
and the deliberate limits of the testing approach.
Chapter~\ref{ch:conclusion} concludes with a critical
discussion, failure and deficiency analysis, and the
extension roadmap. Three appendices provide extended results
...
The next eighteen subsections cover the full literature landscape.
The first eight are canonical anchors in each historical wave and
carry a TikZ pipeline figure (SIFT, PolyPhen-2, REVEL, CADD, VARITY,
EVE, ESM-1b/2, AlphaMissense). The following ten subsections
(Sections~\ref{sec:rw-mvp}--\ref{sec:rw-dyna}) are more recent
neighbours of our approach and are treated narrative-only for space;
they follow the abridged template core idea $\to$ algorithmic
summary $\to$ pros / cons. A final subsection
(Section~\ref{sec:rw-methodology-refs}) cites two non-model
methodology works used as benchmarking references throughout the
thesis. Every subsection follows the same structure -- core idea,
algorithmic summary, and a short discussion of strengths and
weaknesses relative
...
\textbf{Disadvantages.}
\begin{itemize}[leftmargin=2em, itemsep=0.15em]
  \item \emph{Compound contamination through the ensemble.}
    REVEL's meta-learning design compounds \textsc{ClinVar}
    overlap through its 13 base predictors. PolyPhen-2
    (HumDiv/HumVar labels), CADD (simulated-vs-observed
    selection, but with observed variants overlapping
    \textsc{ClinVar}), MutationTaster, and several others
    were all trained on or calibrated against populations
    that overlap the \textsc{ClinVar} test set commonly
    used to benchmark REVEL. The reported ROC-AUC of
    $\sim$$0.94$ is therefore an upper bound that includes
    an irreducible ensemble-level contamination component,
    and the effect cannot be measured without refitting
    each base learner on a strictly disjoint corpus ---
    a resource-prohibitive undertaking.
...
\textbf{Disadvantages.}
\begin{itemize}[leftmargin=2em, itemsep=0.15em]
  \item Coverage at the original release was approximately
    $3{,}000$ genes; subsequent updates have expanded this,
    though the per-gene VAE training cost remains the
    bottleneck for whole-proteome coverage.
...
% ───────────── additional papers (P1-5 of thesis review) ─────────────

\subsection{MVP (2021) \texorpdfstring{\citep{qi2021mvp}}{(Qi et al., 2021)}}
\label{sec:rw-mvp}

\textbf{Core idea.} The Missense Variant Pathogenicity predictor (MVP) is
a residual-network classifier trained on a composite feature matrix of
conservation, biochemistry, and pre-computed VEP scores
\citep{qi2021mvp}.

\textbf{Algorithmic summary.} A ResNet-style architecture (38 layers,
skip connections) maps a 38-dimensional per-variant feature vector to
a single pathogenicity score; separate models are fit for genes with
and without strong evolutionary constraint. Training uses the HGMD
disease panel plus \textsc{ClinVar}. Reported AUROC is $0.91$ on
constrained genes and $0.85$ on non-constrained.

\textbf{Pros.} First published ResNet for this task; demonstrates that
moderately deep architectures outperform shallow classifiers on the
same feature matrix. \textbf{Cons.} Evaluated with random
splits; no paralog guard; feature matrix includes meta-predictors,
which creates circularity when benchmarking on \textsc{ClinVar}.

\subsection{gMVP (2022) \texorpdfstring{\citep{zhang2022gmvp}}{(Zhang et al., 2022)}}
\label{sec:rw-gmvp}

\textbf{Core idea.} Replaces MVP's dense feature pack with a graph
attention network that reasons directly over a co-evolution graph
\citep{zhang2022gmvp}.

\textbf{Algorithmic summary.} Each protein is represented by a
position-by-position graph whose edges are derived from MSA
co-evolution and (when available) predicted contact maps. A graph
attention network aggregates evolutionary context into a
per-position embedding; a small head predicts pathogenicity. Reported
AUROC is $0.88$ on a random split with HGMD + \textsc{ClinVar} labels.

\textbf{Pros.} Introduces explicit relational reasoning over
evolutionary context, a structurally richer representation than
MVP's dense feature pack. \textbf{Cons.} Still random-split
evaluation; no external-cohort validation; no paralog guard.

\subsection{MutFormer (2023) \texorpdfstring{\citep{jiang2023mutformer}}{(Jiang et al., 2023)}}
\label{sec:rw-mutformer}

\textbf{Core idea.} Combines a CNN over the local protein window with a
BERT-style transformer over longer-range sequence context
\citep{jiang2023mutformer}.

\textbf{Algorithmic summary.} A 1D CNN encodes the $\pm 30$-residue
window around the variant; a BERT encoder processes the full
sequence; the two embeddings are concatenated with optional dbNSFP
features before a sigmoid head. Published AUROC is $0.94$ on a
ClinVar-derived benchmark.

\textbf{Pros.} Architecturally closest in spirit to our planned extensions
(hybrid sequence + tabular). \textbf{Cons.} Again random-split; the
hybrid depends on dbNSFP which carries its own contamination risk;
inference cost is nontrivial.

\subsection{mvPPT (2023) \texorpdfstring{\citep{tong2023mvppt}}{(Tong et al., 2023)}}
\label{sec:rw-mvppt}

\textbf{Core idea.} A LightGBM-based missense predictor fit on dbNSFP
+ gnomAD tabular features, including a number of meta-predictor
columns \citep{tong2023mvppt}.

\textbf{Algorithmic summary.} Standard gradient-boosted trees on
dbNSFP conservation, amino-acid chemistry, and population-frequency
features, with a handful of meta-predictor scores retained as
inputs. Reports AUROC $\sim$$0.91$ on a random split.

\textbf{Pros.} Closest peer to our XGBoost baseline in terms of
feature type and algorithm family. \textbf{Cons.} Uses
meta-predictors as features, which are themselves ClinVar-trained
--- this is the precise contamination source our banned-feature
list (Section~\ref{sec:fw-part2}) removes.

\subsection{MAGPIE (2024) \texorpdfstring{\citep{liu2024magpie}}{(Liu et al., 2024)}}
\label{sec:rw-magpie}

\textbf{Core idea.} A gradient-boosted tree model spanning six variant
types (missense, silent, splice, frameshift, indel, stop-gained) in a
single unified head \citep{liu2024magpie}.

\textbf{Algorithmic summary.} Uses $64$ features across conservation,
biochemistry, population frequency, and domain annotations. On the
DS-MVP missense benchmark MAGPIE reports AUROC $0.892$. No
protein-language-model signal is included.

\textbf{Pros.} Unified, interpretable, and multi-variant-type.
\textbf{Cons.} Reports gene-level split only; no paralog guard; no
external-cohort validation.

\subsection{ESM-1v (2021) \texorpdfstring{\citep{meier2021esm1v}}{(Meier et al., 2021)}}
\label{sec:rw-esm1v}

\textbf{Core idea.} The earliest zero-shot protein-language-model
approach to variant-effect prediction, pre-dating ESM-1b and ESM-2
\citep{meier2021esm1v}.

\textbf{Algorithmic summary.} Ensembles five ESM-1v 650M checkpoints
trained on UniRef90; scores every variant by a masked log-likelihood
ratio analogous to the one we use in
Section~\ref{sec:arch-esm2}. Evaluated on 41 DMS assays; no
\textsc{ClinVar} dependence.

\textbf{Pros.} Fully unsupervised; the cleanest possible external
baseline for our rank-fusion experiment. \textbf{Cons.} Large model
(650M $\times$ 5) makes large-corpus scoring expensive on free GPU
budgets.

\subsection{PrimateAI-3D (2023) \texorpdfstring{\citep{gao2023primateai3d}}{(Gao et al., 2023)}}
\label{sec:rw-primateai3d}

\textbf{Core idea.} A 3D CNN that ingests a voxelised representation
of the local protein structure around the variant residue, plus
cross-species variation from 233 primate genomes
\citep{gao2023primateai3d}.

\textbf{Algorithmic summary.} Crops a 3D volume around the variant's
$\text{C}\alpha$, voxelises amino-acid identity and conservation into
per-voxel channels, and passes the tensor through a 3D CNN. The
model is trained to separate variants \emph{observed} in primates
from those \emph{unobserved} (the so-called ``primate-AI'' label),
which is a strictly non-\textsc{ClinVar} signal. Published AUROC on
\textsc{ClinVar} is $0.94$.

\textbf{Pros.} First large-scale structural signal that is \emph{not}
derived from \textsc{ClinVar}. \textbf{Cons.} Training corpus is
proprietary (Illumina primate sequencing); model weights are not
fully redistributable.

\subsection{VariPred (2024) \texorpdfstring{\citep{lin2024varipred}}{(Lin et al., 2024)}}
\label{sec:rw-varipred}

\textbf{Core idea.} A twin-network architecture over ESM-1b embeddings
that scores a reference/alternate pair directly
\citep{lin2024varipred}.

\textbf{Algorithmic summary.} Two parallel ESM-1b passes (one on the
reference sequence, one on the alternate) feed a small
difference-head that outputs a pathogenicity score. A surprising
ablation in the paper shows ESM-1b $>$ ESM-2 for this task at 650M
parameters, which is our baseline motivation for scoring the 35M
ESM-2 as a proof-of-concept rather than the production model.

\textbf{Pros.} Clean twin-network design; published AUROC $0.928$ and
MCC $0.714$. \textbf{Cons.} 650M $\times 2$ inference is expensive;
training requires a labelled \textsc{ClinVar} slice.

\subsection{VespaG (2024) \texorpdfstring{\citep{marquet2024vespag}}{(Marquet et al., 2024)}}
\label{sec:rw-vespag}

\textbf{Core idea.} Distils GEMME (a pure-evolution unsupervised method)
into ESM-2 embeddings, producing a fast, fully-unsupervised predictor
that can score the human proteome in minutes on a single GPU
\citep{marquet2024vespag}.

\textbf{Algorithmic summary.} ESM-2 embeddings are frozen; a light
per-residue head is trained to mimic GEMME's position-specific
scoring matrix. The result is a $\text{GPU-sec}$-per-protein scorer
that preserves evolutionary semantics without re-running an MSA.

\textbf{Pros.} The cheapest published PLM-based scorer; natural
partner for the rank-fusion experiment of Section~\ref{sec:res-esm2}.
\textbf{Cons.} Relies on ESM-2 embeddings being discriminative, which
our sanity tests (Section~\ref{sec:res-esm2}, P0-2) suggest may
degrade at small model sizes.

\subsection{DYNA (2025) \texorpdfstring{\citep{zhan2025dyna}}{(Zhan et al., 2025)}}
\label{sec:rw-dyna}

\textbf{Core idea.} Siamese fine-tuning of ESM over a \emph{disease-
specific} labelled slice, producing a classifier specialised to a
narrow clinical domain \citep{zhan2025dyna}.

\textbf{Algorithmic summary.} A pair of shared-weight ESM branches
processes the reference and alternate sequences; a contrastive
disease-specific loss sharpens the embedding separation. The paper
reports AUROC gains of $>5$ percentage points over ESM-1b on disease-
specific benchmarks (e.g.\ cardiomyopathy, retinopathy).

\textbf{Pros.} Plausible path to clinical deployment where a single
general-purpose classifier underperforms. \textbf{Cons.} Requires a
labelled disease-specific training slice; generalisation outside
that slice is unstudied.

\subsection{Methodology references (not a model):
\texorpdfstring{\citep{livesey2023benchmark_v2, notin2023proteingym_v2}}%
{Livesey \& Marsh 2023; ProteinGym 2023}}
\label{sec:rw-methodology-refs}

Two recent works are cited throughout the thesis but are not
themselves predictors. \citet{livesey2023benchmark_v2} benchmark 55
VEPs against deep mutational scanning (DMS) assays and quantify the
benchmark-inflation effect of \textsc{ClinVar}-only evaluation;
\citet{notin2023proteingym_v2} publish \textsc{ProteinGym}, a
large-scale DMS benchmark with 217 assays that our extension roadmap
(Section~\ref{sec:conc-future}) targets as the next external-cohort
validation after \textsc{denovo-db}.
...
reviewed methods along eight axes relevant to a
methodologically honest evaluation: supervision,
\textsc{ClinVar} contamination risk, calibration,
\emph{paralog-aware split?}, \emph{leakage audit
reported?}, and external-cohort validation. Every
published method we reviewed uses at most a gene-level
split; none publish a paralog-family-level split, and
none publish a systematic leakage audit with
before-and-after numbers. The last two columns of the
table make this explicit.
...
\textbf{Supervision} --- whether the model consumes
clinical pathogenicity labels during training.
\textbf{ClinVar risk} --- whether the published benchmark
is at risk of training-set contamination.
\textbf{Paralog-aware split?} --- whether the publication
enforces a family-level split (not just gene-level).
\textbf{Leakage audit reported?} --- whether the paper
publishes a systematic audit with numerical
before/after deltas for at least one leakage source.
\textbf{External?} --- whether the paper validates on a
cohort not derived from \textsc{ClinVar}.}
...
\textbf{Method} & \textbf{Year} & \textbf{Supervision}
 ...
 & \textbf{Paralog-aware split?} & \textbf{Leakage audit reported?} \\
\midrule
SIFT           & 2003  & unsup.\      & none     & --          & No & No       \\
PolyPhen-2     & 2010  & sup.\        & partial  & soft        & No & No       \\
CADD           & 2014+ & self-sup.\   & none     & phred       & No & No       \\
REVEL          & 2016  & sup.\        & compound & --          & No & No       \\
MVP            & 2021  & sup.\        & compound & --          & No & No       \\
VARITY         & 2021  & sup.\        & partial  & reported    & No (gene only) & Partial \\
EVE            & 2021  & unsup.\      & none     & ELBO only   & No (gene only) & No       \\
ESM-1v         & 2021  & zero-shot    & none     & --          & No             & No       \\
gMVP           & 2022  & sup.\        & partial  & --          & No             & No       \\
MutFormer      & 2023  & sup.\        & partial  & --          & No             & No       \\
mvPPT          & 2023  & sup.\        & compound & --          & No             & No       \\
ESM-1b/ESM-2   & 2023  & zero-shot    & none     & --          & No             & Partial  \\
PrimateAI-3D   & 2023  & self-sup.\   & none     & --          & No             & No       \\
AlphaMissense  & 2023  & sup.\        & high     & calibrated  & No (gene only) & No       \\
VariPred       & 2024  & sup.\        & partial  & --          & No             & No       \\
VespaG         & 2024  & unsup.\      & none     & --          & No             & No       \\
MAGPIE         & 2024  & sup.\        & partial  & --          & No             & No       \\
DYNA           & 2025  & sup.\        & partial  & --          & No             & No       \\
\midrule
\textbf{This work} & \textbf{2026} & \textbf{sup.+zs fusion}
 & \textbf{audited} & \textbf{isotonic}
 & \textbf{Yes (family)} & \textbf{Yes (Section~\ref{sec:res-leakage})} \\
\bottomrule
\end{tabular}%
}
\end{table}

\paragraph{Legend for Table~\ref{tab:rw-consolidated}.}
\emph{Supervision} $\in$ \{unsup., self-sup., sup., zero-shot, sup.+zs fusion\}.
\emph{ClinVar risk} $\in$ \{none, partial, compound, high, audited\};
\emph{none} = no overlap with ClinVar-derived training data;
\emph{partial} = overlap with one or more ClinVar-adjacent
curations (HumDiv, HumVar); \emph{compound} = meta-predictor
composed of ClinVar-trained base tools; \emph{high} = calibrated
directly against \textsc{ClinVar} at release;
\emph{audited} = contamination sources enumerated and
quantitatively removed (this work).
\emph{Calibration} $\in$ \{--, soft, phred, ELBO only, reported,
calibrated, isotonic\}; \emph{--} = no calibration reported,
\emph{soft} = three-way soft label (benign / possibly / probably),
\emph{phred} = Phred-scaled, \emph{ELBO only} = VAE evidence
lower-bound difference, \emph{reported} = reliability diagram
only, \emph{calibrated} = post-hoc calibrator at release,
\emph{isotonic} = isotonic regression fit on held-out val
probabilities.
\emph{Paralog-aware split?} and \emph{Leakage audit reported?}
are binary. ``gene only'' marks methods that achieve a
gene-level split but do not go further to paralog family.

\section{Research Gap}
\label{sec:rw-gap}

Two structural gaps in the published missense-classification
literature motivate this work's methodological focus.

\textbf{First, paralog-disjoint evaluation is rare.} Almost
every published benchmark performs a gene-level random split
or a disease-stratified split, neither of which prevents
highly-similar paralogs (for example, the $109$ keratin
members, the $83$ cadherin members, or the $\sim$$400$
olfactory receptor family) from appearing in both train and
test partitions. When paralogs share $60$--$90\%$ amino-acid
identity, a classifier trained on one paralog can score its
siblings trivially, which inflates benchmark AUROC by a
measurable amount (Section~\ref{sec:res-leakage} of this
thesis quantifies the effect at $+0.081$ ROC-AUC on our
pipeline).

\textbf{Second, external validation on a dataset not derived
from \textsc{ClinVar} is almost never reported.} Variant-
effect predictors that post strong \textsc{ClinVar} numbers
frequently collapse when tasked with ranking \emph{de novo}
variants in affected children versus unaffected siblings
(e.g.\ the \textsc{denovo-db} corpus) because the latter
requires gene-level pathogenicity priors that variant-level
tools do not see. The published literature tends to publish
the pass and omit the fail.

This thesis addresses both gaps directly: we enforce
paralog-family disjointness in continuous integration, and
we report pre- and post-constraint external metrics on
\textsc{denovo-db}.
...
post-constraint numbers separately, and we publish a
paralog-split, transparent, self-contained like-for-like
comparison of our baseline against SIFT, PolyPhen-2, and
AlphaMissense under a single identical evaluation. We do
not claim the methodology is novel in isolation -- independent
benchmarks such as \citet{livesey2023benchmark} cover the
same tools -- but the CI-enforced paralog guard and the
public bundle of pre/post-constraint denovo-db numbers are
rarely combined in one artefact.
...
chart covering Sep~2025--May~2026.} Term~1
(Sep--Dec 2025, blue) covered literature review, data
ingestion, and the first XGBoost baseline. Term~2
(Jan--May~2026, orange), marked by the dashed red vertical
separator, covered the leakage audit, hyperparameter search,
...
\textbf{Schedule realisation.} In practice the realised
schedule departed from the plan in two specific ways. First,
the leakage-audit stage consumed approximately three weeks of
Term~2 rather than the planned one week, because fixing the
initial random-split inflation (Failure F1,
Section~\ref{sec:conc-failures}) required re-running every
downstream experiment. Second, the ESM-2 full-training-set
scoring run was descoped from Phase~1 to Phase~2.1 when the
mid-Term-2 scope lock indicated that the free Colab T4 quota
would not accommodate both the zero-shot proof-of-concept and
the full-training integration within the remaining weeks. The
LaTeX write-up and the zero-shot ESM-2 scoring ran in parallel
during April as planned, which preserved the defence-week
milestone. No schedule slip was allowed to delay the leakage
gate or the reproduce-headline integration test, consistent
with the priority rule stated above.
...
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}|>{\columncolor{green!8}\raggedright\arraybackslash}p{0.48\linewidth}|>{\columncolor{orange!10}\raggedright\arraybackslash}p{0.48\linewidth}|@{}}
\hline
\rowcolor{green!25}\multicolumn{1}{|l|}{\textbf{Strengths (internal, positive)}} &
\cellcolor{orange!25}\textbf{Weaknesses (internal, negative)} \\
\hline
\vspace{-0.5em}\begin{itemize}[leftmargin=1.1em, itemsep=0.15em, topsep=0.2em]
  \item[\textbf{S1}] Paralog-disjoint split + CI-enforced leakage gate.
  \item[\textbf{S2}] Calibration reported with Murphy decomposition, not only AUROC.
  \item[\textbf{S3}] External validation on \textsc{denovo-db}.
  \item[\textbf{S4}] Reproducible: \texttt{docker run} yields identical metrics.
  \item[\textbf{S5}] Open-source, zero paid dependencies.
\end{itemize} &
\vspace{-0.5em}\begin{itemize}[leftmargin=1.1em, itemsep=0.15em, topsep=0.2em]
  \item[\textbf{W1}] ClinVar-only training; no DMS benchmark coverage.
  \item[\textbf{W2}] Static snapshot; no continuous re-training pipeline.
  \item[\textbf{W3}] Single tree classifier; no neural ensemble in Phase~1.
  \item[\textbf{W4}] Interpretability is feature-level (SHAP); no mechanistic/structural explanation.
\end{itemize} \\
\hline
\rowcolor{blue!20}\multicolumn{1}{|l|}{\textbf{Opportunities (external, positive)}} &
\cellcolor{red!20}\textbf{Threats (external, negative)} \\
\hline
\rowcolor{blue!6}
\vspace{-0.5em}\begin{itemize}[leftmargin=1.1em, itemsep=0.15em, topsep=0.2em]
  \item[\textbf{O1}] ESM-2 rank fusion is orthogonal to XGBoost; hybrid gain observed on denovo-db.
  \item[\textbf{O2}] AlphaFold-2 pipeline already wired in for structural features.
  \item[\textbf{O3}] Streamlit UI can be re-hosted on Hugging Face Spaces at zero cost.
  \item[\textbf{O4}] Methodology publishable as a short communication.
\end{itemize} &
\cellcolor{red!6}
\vspace{-0.5em}\begin{itemize}[leftmargin=1.1em, itemsep=0.15em, topsep=0.2em]
  \item[\textbf{T1}] ClinVar label churn between releases.
  \item[\textbf{T2}] Regulatory overhead blocks clinical deployment (SFDA class B+).
  \item[\textbf{T3}] AlphaMissense/AlphaFold-3 may close performance gap in months.
  \item[\textbf{T4}] PLM compute grows with model size; a 650M jump infeasible on free tier.
\end{itemize} \\
\hline
\end{tabular}
...
\begin{tikzpicture}[
  ...
  stage/.style={draw, rounded corners=3pt, minimum width=1.9cm,
    minimum height=0.8cm, align=center, thick,
    font=\sffamily\scriptsize},
  part1/.style={stage, fill=blue!10},
  part2/.style={stage, fill=orange!12},
  part3/.style={stage, fill=green!12},
  artf/.style={draw, rounded corners=3pt, minimum width=1.95cm,
    minimum height=0.7cm, align=center, fill=black!6, thick,
    font=\sffamily\scriptsize\itshape},
  arrow/.style={-{Latex[length=4pt]}, thick},
  lane/.style={font=\sffamily\bfseries\footnotesize, align=center, text width=1.3cm},
]
  % Lane labels (left column)
  \node[lane] (ln1) at (0, 0)    {Part 1\\(data)};
  \node[lane] (ln2) at (0, -1.8) {Part 2\\(training)};
  \node[lane] (ln3) at (0, -3.6) {Part 3\\(evaluation)};

  % Part 1 --- row at y=0
  \node[part1] (ing)  at (2.2, 0) {ClinVar +\\gnomAD};
  \node[part1] (miss) at (4.4, 0) {Missense\\filter};
  \node[part1] (feat) at (6.6, 0) {dbNSFP +\\AF merge};
  \node[part1] (para) at (8.8, 0) {Paralog\\split};
  \node[artf]  (art1) at (11.1, 0) {splits/\\*.parquet};

  % Part 2 --- row at y=-1.8
  \node[part2] (gate) at (2.2, -1.8) {Leakage\\gate};
  \node[part2] (opt)  at (4.4, -1.8) {Optuna\\HPO};
  \node[part2] (thr)  at (6.6, -1.8) {Threshold\\sweep};
  \node[part2] (cal)  at (8.8, -1.8) {Isotonic\\calibration};
  \node[artf]  (art2) at (11.1, -1.8) {model.json};

  % Part 3 --- row at y=-3.6
  \node[part3] (boot) at (2.2, -3.6) {Bootstrap\\CIs};
  \node[part3] (shap) at (4.4, -3.6) {SHAP};
  \node[part3] (base) at (6.6, -3.6) {Baselines\\re-score};
  \node[part3] (ext)  at (8.8, -3.6) {denovo-db +\\ESM-2 LLR};
  \node[artf]  (art3) at (11.1, -3.6) {metrics/\\*.csv};
  ...
  % Cross-row artifact flow (dashed red)
  \draw[arrow, dashed, red!70] (art1.south) -- ++(0,-0.3) -| (gate.north);
  \draw[arrow, dashed, red!70] (art2.south) -- ++(0,-0.3) -| (boot.north);
\end{tikzpicture}
...
\paragraph{Why 39 and not more?} REVEL combines 13
meta-predictors and CADD uses $>\!1{,}400$ features, so a
reviewer may reasonably ask why our feature set is so
small. Two deliberate decisions explain the count.
\emph{First}, every meta-predictor trained on
\textsc{ClinVar} is excluded: using, e.g., REVEL or
\textsc{ClinPred} as an input would make the benchmark
circular, since those tools themselves train on a
snapshot of the same labels we are trying to predict
(Section~\ref{sec:intro-contributions}, contribution
\#1). This rules out more than fifteen scores that would
otherwise inflate the headline number. \emph{Second}, we
prefer singly-sourced, individually-interpretable
features over dense feature packs: every SHAP row in
Table~\ref{tab:shap} maps to a column the reader can
trace back to a named source (phyloP100way, BLOSUM62,
LOEUF, etc.), which is what makes the
quality-attribute trade-off of
Section~\ref{sec:fw-quality} (interpretability vs.\
predictive performance) resolvable in favour of
interpretability without loss of performance. The
feature-group ablation in Section~\ref{sec:res-ablation}
confirms this trade-off is cheap: conservation alone
carries almost all the signal, so pack size would
...
leakage it removes (Section~\ref{sec:res-leakage}).
...
$70/15/15$ ratio. This design is \emph{strictly stronger
than the gene-level split} that \citet{heijl2020gap}
propose as the minimum acceptable evaluation standard
for variant-effect predictors: a gene-level split alone
still lets paralogous families straddle train and test,
and their benchmark over 55 published VEPs shows that
random splitting inflates AUROC by 5--15 percentage
points on this task.
\end{enumerate}
...
\begin{tikzpicture}[
  font=\sffamily\footnotesize,
  ckpt/.style={draw, rounded corners=3pt, minimum width=2.8cm,
    minimum height=0.85cm, align=center, fill=orange!25, thick,
    font=\sffamily\scriptsize\bfseries},
  track/.style={draw, rounded corners=3pt, minimum width=4.2cm,
    minimum height=0.7cm, align=center, fill=green!12, thick,
    font=\sffamily\scriptsize},
  art/.style={draw, rounded corners=3pt, minimum width=3.4cm,
    minimum height=0.6cm, align=center, fill=black!6,
    font=\sffamily\scriptsize\ttfamily, thick},
  arrow/.style={-{Latex[length=5pt]}, thick},
]
  % Left column: trained checkpoint
  \node[ckpt] (m) at (0, 0) {Trained\\checkpoint\\ (\texttt{model.json})};

  % Middle column: 5 tracks (1.0 cm apart vertically)
  \node[track] (t1) at (4.8,  2.0) {1. Bootstrap CIs};
  \node[track] (t2) at (4.8,  1.0) {2. TreeSHAP attribution};
  \node[track] (t3) at (4.8,  0.0) {3. SIFT / PolyPhen / AM};
  \node[track] (t4) at (4.8, -1.0) {4. denovo-db external};
  \node[track] (t5) at (4.8, -2.0) {5. ESM-2 zero-shot LLR};

  % Right column: artifacts
  \node[art] (a1) at (10.3,  2.0) {bootstrap\_ci.csv};
  \node[art] (a2) at (10.3,  1.0) {shap\_values.pq};
  \node[art] (a3) at (10.3,  0.0) {baselines.csv};
  \node[art] (a4) at (10.3, -1.0) {denovo\_metrics.csv};
  \node[art] (a5) at (10.3, -2.0) {esm2\_llr.pq};

  % Checkpoint -> tracks (fan-out)
  \foreach \t in {t1, t2, t3, t4, t5} \draw[arrow] (m.east) -- (\t.west);
  % Tracks -> artifacts (horizontal)
  \foreach \x in {1, 2, 3, 4, 5} \draw[arrow] (t\x.east) -- (a\x.west);
\end{tikzpicture}
...
\begin{tikzpicture}[
  font=\sffamily\scriptsize,
  layer/.style={draw, rounded corners=3pt, minimum width=9.0cm,
    minimum height=0.85cm, align=center, thick},
  cross/.style={draw, rounded corners=3pt, minimum width=2.1cm,
    minimum height=3.0cm, align=center, thick,
    font=\sffamily\scriptsize},
]
  % Main 6 layers, centred
  \node[layer, fill=red!10]                         (l6) at (0,  5)   {\textbf{L6~~Presentation} \ -- \ Streamlit \textbullet\ Typer CLI \textbullet\ Python API};
  \node[layer, fill=orange!12]                      (l5) at (0,  4)   {\textbf{L5~~Application} \ -- \ validation \textbullet\ annotation cache \textbullet\ payload};
  \node[layer, fill=yellow!20]                      (l4) at (0,  3)   {\textbf{L4~~Service} \ -- \ score\_variant() \textbullet\ explain() \textbullet\ batch\_score()};
  \node[layer, fill=green!12]                       (l3) at (0,  2)   {\textbf{L3~~ML core} \ -- \ XGBoost \textbullet\ isotonic \textbullet\ SHAP \textbullet\ ESM-2 LLR};
  \node[layer, fill=blue!10]                        (l2) at (0,  1)   {\textbf{L2~~Data access} \ -- \ \texttt{pyarrow} \textbullet\ paralog splitter \textbullet\ assembler};
  \node[layer, fill=black!6]                        (l1) at (0,  0)   {\textbf{L1~~Data} \ -- \ ClinVar \textbullet\ gnomAD \textbullet\ dbNSFP \textbullet\ denovo-db \textbullet\ UniProt};

  % Cross-cutting concerns (left and right bands aligned to layer stack)
  \node[cross, fill=red!6]  at (-5.95,  2.5)
        {\textbf{Quality}\\\textbf{gates}\\[3pt]\texttt{ruff}\\\texttt{mypy}\\\texttt{pytest}\\leakage gate\\CI on PR};
  \node[cross, fill=teal!12] at ( 5.95,  2.5)
        {\textbf{Repro-}\\\textbf{ducibility}\\[3pt]Docker\\\texttt{uv.lock}\\fixed seed\\immutable\\data snaps};
\end{tikzpicture}
...
\begin{description}[style=nextline,leftmargin=2em]
\item[NFR1 -- Performance.] Score latency is
\emph{targeted} at below $100$ milliseconds for cache hits
and below $2$ seconds for cache misses
(\textsc{VEP} REST round-trip). Observed performance is
dominated by the XGBoost \texttt{predict\_proba} call
($\sim$$10$~ms for a single row) plus the
\textsc{VEP} REST round-trip, which together have
historically sat inside both targets in informal testing;
formal latency benchmarking with $p50/p95/p99$ statistics
is deferred to the deployment-hardening phase
(Deficiency D6, Section~\ref{sec:conc-future}).
...
\begin{tikzpicture}[
  ...
  \draw[inc] (u7) -- (u8) node[midway, above, font=\sffamily\tiny, gray!70]
       {\textit{\textlangle include\textrangle}};
\end{tikzpicture}}
\caption[Use case diagram]{\textbf{Use-case view of the
system.} Two human actors (Clinical Geneticist, Research
Scientist) share the core scoring use cases UC1--UC5,
reflecting the stakeholder-to-surface mapping of
Table~\ref{tab:stakeholders}. The \textsc{VEP} REST Service
is a non-human secondary actor invoked only when UC2
experiences a cache miss; the CI/CD System exercises UC8 on
every push to enforce the leakage gate
(Algorithm~\ref{alg:leakage-gate}). The
$\langle\!\langle include\rangle\!\rangle$ relationship on
UC1 makes it explicit that every score call passes through
calibration and feature-attribution steps. Solid lines are
associations; dashed lines are
$\langle\!\langle include\rangle\!\rangle$ relationships;
dotted lines are $\langle\!\langle extend\rangle\!\rangle$.
\textbf{The use cases map to capabilities realised across
multiple modules in \texttt{src/} and \texttt{scripts/}};
UC2 (Fetch annotation), UC3 (Calibrate), and UC4 (Explain)
are not implemented as separate subsystems but as
functions distributed within the production code.}
\label{fig:uc}
\end{figure}
...
Figure~\ref{fig:class} shows a conceptual class diagram of
the \texttt{src/} package. Classes are grouped into four
coloured \emph{packages} corresponding to the actual
sub-directory structure: \texttt{src/} (root, where the
flat data-pipeline modules live), \texttt{src/models/},
\texttt{src/baselines/} combined with
\texttt{src/external\_validation/} (eval extensions), and
\texttt{scripts/} \citep{martin2017cleanarch}. Each class
is rendered as a three-compartment rectangle: the class
name on top, the key attributes in the middle, and the
principal methods at the bottom. Associations carry
multiplicities; the \texttt{LeakageGate} class has a
\emph{composition} (filled diamond) with each of its
\textbf{six} invariant checkers (banned features, missense
filter, paralog (gene + family) disjointness, label-rate
gap, Phase-2.1 ESM-2 banned-upstream, and ESM-2 split
integrity) because the gate does not exist meaningfully
without them. \textbf{Production code uses module-level
functions in \texttt{src/}* rather than the named classes
shown here}; the diagram is a logical blueprint of the
architectural relationships, not an implementation map,
following Python convention for ML pipelines.
...
\begin{tikzpicture}[
  ...
  cls/.style={draw, rectangle split, rectangle split parts=3,
    ...
    thick, align=left, text width=3.3cm, inner sep=2pt,
  ...
  pkg/.style={draw, dashed, rounded corners, inner sep=0.25cm,
    fill opacity=0.04, text opacity=1},
  rel/.style={-{Latex[length=5pt]}, thick},
  comp/.style={-{Diamond[length=6pt, width=6pt, fill=black]}, thick},
  node distance=0.45cm and 0.45cm,
]
  % Row 1 --- Data + Model packages
  \node[data] (load) {\textbf{VariantLoader}
        \nodepart{second}\scriptsize clinvar\_url: Path
        \nodepart{third}\scriptsize +load(): DataFrame};
  \node[data, below=of load] (feat) {\textbf{FeatureAssembler}
        \nodepart{second}\scriptsize banned: Set[str]
        \nodepart{third}\scriptsize +assemble(df)};
  \node[data, below=of feat] (split) {\textbf{ParalogSplitter}
        \nodepart{second}\scriptsize families: Dict\\ seed: int
        \nodepart{third}\scriptsize +split(df)};

  \node[model, right=1.1cm of load] (xgb) {\textbf{XGBoostTrainer}
        \nodepart{second}\scriptsize params: dict\\ booster: Booster
        \nodepart{third}\scriptsize +fit(X, y)\\ +predict\_proba(X)};
  \node[model, below=of xgb] (cal) {\textbf{IsotonicCalibrator}
        \nodepart{second}\scriptsize mapping: Iso
        \nodepart{third}\scriptsize +fit(p, y)\\ +transform(p)};
  \node[model, below=of cal] (esm) {\textbf{ESM2Scorer}
        \nodepart{second}\scriptsize hf\_name: str\\ device: str
        \nodepart{third}\scriptsize +score(query)};

  % Row 2 (below) --- Eval + Interface packages
  \node[eval, below=1.6cm of split] (boot) {\textbf{BootstrapMetrics}
        \nodepart{second}\scriptsize n\_boot: int = 1000
        \nodepart{third}\scriptsize +compute(y, p)};
  \node[eval, right=of boot] (shap) {\textbf{SHAPExplainer}
        \nodepart{second}\scriptsize sample\_size: int
        \nodepart{third}\scriptsize +explain(X, model)};
  \node[eval, right=of shap] (gate) {\textbf{LeakageGate}
        \nodepart{second}\scriptsize checks: List[Check]
        \nodepart{third}\scriptsize +run\_all(): Report};

  \node[iface, below=of boot]  (app)  {\textbf{ScoreService}
        \nodepart{second}\scriptsize model, cal, shap
        \nodepart{third}\scriptsize +score\_variant(v)};
  \node[iface, below=of shap]  (web)  {\textbf{StreamlitApp}
        \nodepart{second}\scriptsize service: ScoreService
        \nodepart{third}\scriptsize +render()};
  \node[iface, below=of gate]  (cli)  {\textbf{TyperCLI}
        \nodepart{second}\scriptsize service: ScoreService
        \nodepart{third}\scriptsize +batch\_score(path)};

  % Package labels (real paths from src/, P0-1 fix)
  \node[font=\sffamily\footnotesize\bfseries, blue!60!black, above=0.15cm of load] {\texttt{src/} (root)};
  \node[font=\sffamily\footnotesize\bfseries, green!50!black, above=0.15cm of xgb] {\texttt{src/models/}};
  \node[font=\sffamily\footnotesize\bfseries, orange!70!black, above=0.15cm of shap, align=center]
       {\texttt{src/baselines/} +\\\texttt{src/external\_validation/}};
  \node[font=\sffamily\footnotesize\bfseries, red!60!black, above=0.15cm of web] {\texttt{scripts/}};

  % Associations (solid arrows)
  \draw[rel] (load) -- (feat);
  \draw[rel] (feat) -- (split);
  \draw[rel] (split.east) to[out=0, in=180] (xgb.west);
  \draw[rel] (xgb) -- (cal);
  \draw[rel] (cal) -- (esm);
  \draw[rel] (xgb.south) to[out=-90, in=90] (boot.north);
  \draw[rel] (esm.south) to[out=-90, in=90] (shap.north);
  \draw[rel] (app.north east) to[out=45, in=-135] (xgb.south west);
  \draw[rel] (web) -- (app);
  \draw[rel] (cli) -- (app);

  % Composition diamond: LeakageGate owns invariant checks
  \draw[comp] (gate.west) -- ++(-0.35, 0);
\end{tikzpicture}
\caption[Class diagram]{\textbf{Class diagram (conceptual).}
Four coloured packages align with the actual \texttt{src/}
sub-directory structure: \texttt{src/} (root, flat data
modules), \texttt{src/models/} (XGBoost, ESM-2),
\texttt{src/baselines/} + \texttt{src/external\_validation/}
(eval extensions), and \texttt{scripts/} (UI surfaces). A
filled diamond denotes composition: \texttt{LeakageGate}
owns the life of its \textbf{six} invariant checks. Open
arrows are directed associations with the \emph{uses /
produces} roles labelled inline. \textbf{Production code
uses module-level functions rather than the named classes
shown here}, following Python convention for ML pipelines.}
\label{fig:class}
\end{figure}
...
\begin{tikzpicture}[
  ...
  participant/.style={draw, rounded corners=2pt, fill=blue!10,
    minimum width=2.0cm, minimum height=0.65cm, thick, align=center,
    font=\sffamily\scriptsize\bfseries},
  lifeline/.style={dashed, gray!50},
  activation/.style={fill=orange!25, draw=orange!60, thin},
  msg/.style={-{Latex[length=5pt]}, thick},
  ret/.style={-{Latex[length=5pt]}, dashed, gray!80, thick},
  mlab/.style={midway, above=0pt, font=\sffamily\scriptsize, fill=white, inner sep=1pt},
  frag/.style={draw=red!60, thick, rounded corners=1.5pt},
]
  % Participants across \linewidth: 5 columns evenly spaced over 12cm
  \node[participant] (a) at (0,    0) {StreamlitApp};
  \node[participant] (s) at (3.0,  0) {ScoreService};
  \node[participant] (c) at (6.0,  0) {FeatureCache};
  \node[participant] (m) at (9.0,  0) {XGBoostTrainer};
  \node[participant] (e) at (12.0, 0) {SHAPExplainer};

  \foreach \p in {a, s, c, m, e} \draw[lifeline] (\p.south) -- ++(0, -8.2);

  % Activation bars
  \fill[activation] ($(a.south)+(-0.07,0)$) rectangle ++(0.14, -8.0);
  \fill[activation] ($(s.south)+(-0.07,-0.7)$) rectangle ++(0.14, -7.0);
  \fill[activation] ($(c.south)+(-0.07,-1.4)$) rectangle ++(0.14, -1.0);
  \fill[activation] ($(c.south)+(-0.07,-3.6)$) rectangle ++(0.14, -0.7);
  \fill[activation] ($(m.south)+(-0.07,-5.1)$) rectangle ++(0.14, -0.7);
  \fill[activation] ($(e.south)+(-0.07,-6.5)$) rectangle ++(0.14, -0.7);

  % Messages (y positions spaced 1.0cm apart for clarity)
  \draw[msg] ($(a.south)+(0.07,-0.7)$) -- ($(s.south)+(-0.07,-0.7)$)
       node[mlab] {\texttt{score\_variant(v)}};
  \draw[msg] ($(s.south)+(0.07,-1.4)$) -- ($(c.south)+(-0.07,-1.4)$)
       node[mlab] {\texttt{lookup(v)}};

  % opt-fragment for cache miss
  \draw[frag] (4.0, -2.1) rectangle (12.8, -3.8);
  \node[anchor=north west, font=\sffamily\scriptsize\bfseries, red!70]
       at (4.0, -2.05) {opt [cache miss]};
  \draw[msg] ($(c.south)+(-0.07,-2.7)$) -- ($(c.south)+(-4.0,-2.7)$)
       node[mlab] {\texttt{VEP.annotate(v)}};
  \node[font=\sffamily\tiny, red!70, anchor=east] at (2.0, -2.95) {REST call (1--2\,s)};
  \draw[ret] ($(c.south)+(-4.0,-3.4)$) -- ($(c.south)+(-0.07,-3.4)$)
       node[mlab] {annotated};

  % back to normal flow
  \draw[msg] ($(c.south)+(0.07,-4.3)$) -- ($(s.south)+(-0.07,-4.3)$)
       node[mlab] {features};
  \draw[msg] ($(s.south)+(0.07,-5.1)$) -- ($(m.south)+(-0.07,-5.1)$)
       node[mlab] {\texttt{predict\_proba(X)}};
  \draw[ret] ($(m.south)+(-0.07,-5.75)$) -- ($(s.south)+(0.07,-5.75)$)
       node[mlab] {p\_raw};

  % self-loop for calibration
  \draw[msg, rounded corners=2pt] ($(s.south)+(0.07,-6.0)$) -- ++(0.6,0) -- ++(0,-0.4) -- ($(s.south)+(0.07,-6.4)$);
  \node[anchor=west, font=\sffamily\scriptsize] at ($(s.south)+(0.7,-6.2)$) {\texttt{calibrate(p)}};

  \draw[msg] ($(s.south)+(0.07,-6.8)$) -- ($(e.south)+(-0.07,-6.8)$)
       node[mlab] {\texttt{explain(X)}};
  \draw[ret] ($(e.south)+(-0.07,-7.5)$) -- ($(s.south)+(0.07,-7.5)$)
       node[mlab] {shap\_vals};

  \draw[ret] ($(s.south)+(-0.07,-8.0)$) -- ($(a.south)+(0.07,-8.0)$)
       node[mlab] {\{p\_cal, band, shap\}};
\end{tikzpicture}
\caption[Sequence diagram]{\textbf{Sequence diagram for a
single scoring request.} Orange bars are activation
frames; the red frame is an \texttt{opt} fragment for the
cache-miss branch (adds $\sim$1--2~s). Self-loop on
\texttt{ScoreService} denotes isotonic calibration.
\textbf{Conceptual blueprint --- the named participants
(\texttt{ScoreService}, \texttt{FeatureCache}, etc.)\ map
to module-level functions in
\texttt{scripts/streamlit\_app.py} plus Pickle/Parquet
on-disk caching}, not concrete classes.}
\label{fig:seq}
\end{figure}
...
pipeline: the leakage gate runs \textbf{six checks}
concurrently (fork-1 / join-1) --- banned features, missense
filter, paralog (gene + family) disjointness, label-rate
gap, Phase-2.1 ESM-2 banned-upstream, and ESM-2 split
integrity --- and the evaluation step runs five
...
\begin{tikzpicture}[
  ...
  stop/.style={circle, draw=black, double, minimum size=0.4cm, inner sep=0pt,
    fill=black!15},
  act/.style={draw, rounded corners=8pt, minimum width=2.6cm,
    minimum height=0.6cm, align=center, thick, fill=blue!8,
    font=\sffamily\scriptsize},
  dec/.style={diamond, draw, minimum width=0.9cm, minimum height=0.7cm,
    inner sep=0pt, fill=orange!10, thick, font=\sffamily\scriptsize, align=center},
  fork/.style={rectangle, draw=black, fill=black, minimum height=0.12cm,
    minimum width=10.5cm, inner sep=0pt},
  arrow/.style={-{Latex[length=5pt]}, thick},
]
  % Top
  \node[start] (s) at (0, 0) {};
  \node[act, below=0.35cm of s] (load) {Load paralog-disjoint splits};

  % Fork 1 --- leakage gate (6 parallel checks, P0-2 fix)
  \node[fork, below=0.45cm of load] (f1) {};
  \node[act, below=0.55cm of f1, xshift=-4.5cm, minimum width=1.7cm] (ch1) {Banned features};
  \node[act, below=0.55cm of f1, xshift=-2.7cm, minimum width=1.7cm] (ch2) {100\% missense};
  \node[act, below=0.55cm of f1, xshift=-0.9cm, minimum width=1.7cm] (ch3) {Paralog disjoint};
  \node[act, below=0.55cm of f1, xshift=0.9cm,  minimum width=1.7cm] (ch4) {Rate gap};
  \node[act, below=0.55cm of f1, xshift=2.7cm,  minimum width=1.7cm] (ch5) {ESM-2 banned};
  \node[act, below=0.55cm of f1, xshift=4.5cm,  minimum width=1.7cm] (ch6) {ESM-2 split};
  \node[fork, below=1.7cm of f1] (j1) {};

  \node[dec, below=0.45cm of j1] (d1) {Pass?};
  \node[act, right=1.3cm of d1, fill=red!12] (halt) {Halt + report};

  \node[act, below=0.45cm of d1] (opt) {Optuna HPO (40 trials)};
  \node[act, below=0.35cm of opt] (thr) {Threshold sweep};
  \node[act, below=0.35cm of thr] (cal) {Isotonic calibration};

  % Fork 2 --- evaluation tracks (5 parallel)
  \node[fork, below=0.45cm of cal] (f2) {};
  \node[act, below=0.55cm of f2, xshift=-4.5cm] (ta) {Bootstrap CIs};
  \node[act, below=0.55cm of f2, xshift=-2.25cm] (tb) {SHAP};
  \node[act, below=0.55cm of f2, xshift=0cm]    (tc) {Baselines};
  \node[act, below=0.55cm of f2, xshift=2.25cm] (td) {denovo-db};
  \node[act, below=0.55cm of f2, xshift=4.5cm]  (te) {ESM-2 LLR};
  \node[fork, below=1.7cm of f2] (j2) {};

  \node[act, below=0.45cm of j2] (rep) {Write \texttt{results/metrics/*.csv}};
  \node[stop, below=0.4cm of rep] (endn) {};
  ...
  \draw[arrow] (d1) -- (opt) node[midway, right, font=\sffamily\scriptsize, xshift=1pt]{yes};
  \draw[arrow] (opt) -- (thr);
  \draw[arrow] (thr) -- (cal);
  \draw[arrow] (cal) -- (f2);
...
are fork/join pseudo-nodes: Fork-1 runs the \textbf{six}
leakage checks in parallel; Fork-2 runs the five evaluation
tracks in parallel. If any leakage check fails, execution
halts without touching the training stage.
\textbf{Conceptual blueprint --- production uses sequential
script invocation, not a true parallel orchestrator}; the
fork/join nodes denote logical independence rather than
...
\end{tikzpicture}}
\caption[State diagram]{\textbf{Per-variant state-machine
view.} The five-state happy path
(Submitted $\rightarrow$ Annotated $\rightarrow$ Scored
$\rightarrow$ Calibrated $\rightarrow$ Classified) matches
the five-step data flow of
Section~\ref{sec:fw-framework}. The dashed red transitions
to \texttt{Failed} capture the two distinct error modes
reported to the user interface: upstream \textsc{VEP} REST
errors (when the feature assembler cannot resolve a
coordinate) and downstream NaN errors (when a required
feature is missing after the cache fallback). The absence
of any transition from \texttt{Classified} back to earlier
states reflects the pipeline's append-only contract:
re-scoring requires a fresh request. The terminal
double-circle indicates the final reported state.}
\label{fig:state}
\end{figure}
...
\begin{tikzpicture}[
  font=\sffamily\footnotesize,
  comp/.style={draw, thick, rounded corners=2pt, minimum width=2.4cm,
    minimum height=0.7cm, align=center, fill=blue!8,
    font=\sffamily\scriptsize},
  ext/.style={draw, thick, rounded corners=10pt, minimum width=2.4cm,
    minimum height=0.7cm, fill=gray!15, align=center,
    font=\sffamily\scriptsize},
  arrow/.style={-{Latex[length=5pt]}, thick},
  dep/.style={-{Latex[length=5pt]}, dashed, gray!70, thick},
]
  % Grid layout: columns at x = 0, 3.5, 7.0, 10.5; rows at y = 3, 1.5, 0, -1.5
  % Left column: UI
  \node[comp]  (sl)   at (0,    3)   {Streamlit UI};
  \node[comp]  (cli)  at (0,    1.5) {Typer CLI};

  % Column 2: service
  \node[comp]  (svc)  at (3.5,  2.25) {ScoreService};

  % Column 3: core modules
  \node[comp]  (mdl)  at (7.0,  3)    {Model (XGBoost)};
  \node[comp]  (cal)  at (7.0,  1.5)  {IsotonicCalibrator};
  \node[comp]  (shap) at (7.0,  0.0)  {SHAPExplainer};
  \node[comp]  (data) at (3.5,  0.0)  {FeatureAssembler};
  \node[comp]  (cache)at (3.5, -1.5)  {Parquet cache};

  % Column 4: external services
  \node[ext]   (vep)  at (11.0,  3)   {VEP REST};
  \node[ext]   (hf)   at (11.0,  1.5) {HuggingFace Hub};
  \node[ext]   (gnom) at (11.0,  0.0) {gnomAD FTP};
  ...
  % External network arrows (dashed)
  \draw[dep] (mdl.east)  -- (vep.west)  node[midway, above, font=\sffamily\tiny, fill=white, inner sep=1pt]{HTTP};
  \draw[dep] (cal.east)  -- (hf.west)   node[midway, above, font=\sffamily\tiny, fill=white, inner sep=1pt]{HTTPS};
  \draw[dep] (shap.east) -- (gnom.west) node[midway, above, font=\sffamily\tiny, fill=white, inner sep=1pt]{FTP};
\end{tikzpicture}
...
arrows are network dependencies. This diagram complements
Figure~\ref{fig:arch} of Chapter~\ref{ch:framework}:
Figure~\ref{fig:arch} shows the logical six-layer
stack, whereas this figure shows the concrete Python
modules and their network dependencies.}
\label{fig:comp}
\end{figure}
...
variant. Calibrated probability is displayed alongside the
three-band classification from functional requirement FR4
(Section~\ref{sec:an-fr}): \emph{benign} if $p < 0.25$,
\emph{uncertain} if $0.25 \le p < 0.75$,
\emph{pathogenic} if $p \ge 0.75$. Probabilities that fall
inside the $[0.25, 0.75)$ band are therefore reported as
\emph{uncertain}, consistent with the interface's advisory
role (the system is not a diagnostic device ---
Section~\ref{sec:feas-op}). Red SHAP bars push the
prediction towards pathogenic, green bars towards benign.}
\label{fig:streamlit}
\end{figure}
...
T4 GPU session for the ESM-2 full-training-
set scoring.
...
($1{,}000$ replicates, seed $= 42$). ROC-AUC and PR-AUC
are reported separately on raw and isotonic-calibrated
probabilities; Brier, ECE, and MCC use calibrated
probabilities. The canonical headline PR-AUC quoted in
the abstract and in the comparison text is the
\emph{calibrated} value of $0.8273$. Uncalibrated
numbers coincide with Appendix~A Table~\ref{tab:full-boot}.}
\label{tab:headline}
\small
\begin{tabular}{@{} l r l @{}}
\toprule
...
\midrule
\multicolumn{3}{@{}l}{\emph{Raw probabilities}} \\
ROC-AUC (raw)        & 0.9378 & $[0.9348, 0.9407]$ \\
PR-AUC (raw)         & 0.8355 & $[0.8265, 0.8434]$ \\
Brier loss (raw)     & 0.0876 & $[0.0853, 0.0899]$ \\
\midrule
\multicolumn{3}{@{}l}{\emph{Isotonic-calibrated probabilities (canonical)}} \\
ROC-AUC (calibrated) & 0.9376 & $[0.9346, 0.9404]$ \\
\textbf{PR-AUC (calibrated)} & \textbf{0.8273} & $\mathbf{[0.8185, 0.8352]}$ \\
...
from $0.955$ to $0.836$ raw (equivalently $0.827$
isotonic-calibrated) PR-AUC. Each stage corresponds
to one distinct, removable leakage source. The first
stage --- replacing the random-split baseline with a
family-level \texttt{GroupShuffleSplit} --- is motivated
directly by the \citet{heijl2020gap} benchmark of
$55$ variant-effect predictors, which quantifies a
$5$--$15$ percentage-point AUROC inflation under random
splitting on this task.
...
misleading PR-AUC of $0.955$ to a defensible $0.836$
(raw) / $0.827$ (calibrated; headline value used in
the abstract and Section~\ref{sec:res-headline}).
...
Table~\ref{tab:baselines} reports the published
baselines scored on our paralog-disjoint test split.
\textbf{Important caveat:} each baseline scores a
different subset of variants -- SIFT covers $96\%$,
PolyPhen-2 $93\%$, AlphaMissense $86\%$, and our
XGBoost $100\%$ -- so the ROC-AUC / PR-AUC values
below are each computed on the model's \emph{own
coverage} subset and are not directly comparable.
Direct subtraction between rows (for example, the
nominal $-0.05$ PR-AUC gap to AlphaMissense) is
\emph{not} a like-for-like gap, since AlphaMissense
reports on the easier 86\,\% of our variants. A
strict matched-coverage evaluation on
$\mathcal{S}_{\text{AM}}$ (the $\sim$24,060 variants
that AlphaMissense can score) together with a paired
DeLong test is coded in
\texttt{scripts/run\_baselines\_matched.py} and is
listed as \textbf{deficiency D7} in
Section~\ref{sec:conc-deficiencies}; it could not be
run at submission time because the raw AlphaMissense
TSV ($\sim$8\,GB) is not checked into the repository
and the trained XGBoost test predictions were not
persisted alongside the checkpoint.

\begin{table}[H]
\centering
\caption[Baseline comparison]{Published baselines scored
on our paralog-disjoint test split. Each row uses that
baseline's own coverage subset, so rows are NOT directly
comparable (see the paragraph above and D7 in
Section~\ref{sec:conc-deficiencies}).}
\label{tab:baselines}
\small
\begin{tabular}{@{} l r l l r @{}}
\toprule
\textbf{Baseline} & \textbf{Year} & \textbf{ROC-AUC (own cov., 95\% CI)} & \textbf{PR-AUC (own cov., 95\% CI)} & \textbf{Coverage} \\
\midrule
SIFT                    & 2003 & 0.881 $[0.877, 0.885]$ & 0.620 $[0.610, 0.629]$ & 96\% \\
PolyPhen-2$^{\dagger}$  & 2010 & 0.893 $[0.888, 0.898]$ & 0.728 $[0.716, 0.739]$ & 93\% \\
\textbf{XGBoost (ours, calibrated)} & 2026 & \textbf{0.938 $[0.935, 0.941]$} & \textbf{0.827 $[0.819, 0.835]$} & \textbf{100\%} \\
AlphaMissense$^{*}$     & 2023 & 0.956 $[0.953, 0.958]$ & 0.890 $[0.882, 0.898]$ & 86\% \\
\bottomrule
\multicolumn{5}{p{0.95\linewidth}}{\footnotesize Our XGBoost row is the isotonic-calibrated probability (the canonical headline, $0.827$); the uncalibrated PR-AUC is $0.836$ $[0.827, 0.843]$. $^{*}$ AlphaMissense was calibrated against \textsc{ClinVar} at release; any ClinVar-derived test set overlaps with its calibration signal. $^{\dagger}$ PolyPhen-2 was trained on HumDiv/HumVar, which overlap with \textsc{ClinVar}. Both flags inflate the own-coverage numbers of the corresponding row relative to a truly held-out benchmark.} \\
\end{tabular}
\end{table}
...
\caption[Baselines forest plot (own-coverage)]{Forest plot of our
XGBoost against three published baselines. \textbf{Each row is on its
model's own coverage subset} (SIFT 96\,\%, PolyPhen-2 93\,\%,
AlphaMissense 86\,\%, XGBoost 100\,\%), so the horizontal spacing
between rows is \emph{not} a like-for-like gap. A matched-coverage
version (restricted to the AM-covered subset) is coded in
\texttt{scripts/run\_baselines\_matched.py} and is deferred to
deficiency D7 (Section~\ref{sec:conc-deficiencies}) until the raw AM
TSV and the XGBoost test-prediction parquet are persisted. Bootstrap
95\,\% CIs are shown for each baseline.}
\label{fig:forestplot}
\end{figure}

\paragraph{The intersection-subset comparison that would close
the coverage confound.} A natural companion to the own-coverage
rows of Table~\ref{tab:baselines} is an \emph{intersection-only}
evaluation restricted to variants scored by all four methods
simultaneously ($\mathcal{S}_{\cap}$), so that the four rows
become strictly comparable on the same sample. Any remaining gap
between AlphaMissense and our XGBoost on $\mathcal{S}_{\cap}$
would be attributable to the method rather than to coverage; any
closure would reveal the nominal $-0.05$ PR-AUC gap as a coverage
artefact. The relevant driver is coded in
\texttt{scripts/run\_baselines\_matched.py}. The intersection-set
rerun is subsumed by deficiency D7 (per-variant XGBoost test
predictions are required for the paired comparison on
$\mathcal{S}_{\text{AM}}$, and these were not persisted at
submission); it is the single most informative numerical
revision item and is the first entry on the future-work
list of Section~\ref{sec:conc-future}.
...
Table~\ref{tab:ablation} reports the effect of removing each of the
six feature groups from Figure~\ref{fig:feature-groups} and retraining
XGBoost with the Optuna-tuned hyperparameters frozen (no per-ablation
re-tune). The $\Delta$ columns are full-baseline $-$ ablated, so
positive $\Delta$ ROC-AUC means the group \emph{carries} signal;
95\,\% confidence intervals come from a $500$-replicate paired case-
resampling bootstrap on the test split. The ablation is reproducible
via \texttt{scripts/run\_ablation.py}; the full CSV lives at
\texttt{results/metrics/ablation.csv} and the accompanying bar chart
at \texttt{results/figures/ablation.png}.

\begin{table}[H]
\centering
\caption[Feature-group ablation]{Feature-group ablation on the
paralog-disjoint test split. Full-baseline ROC-AUC is $0.939$ and
PR-AUC is $0.834$. Conservation is by far the dominant group; AA
identity, physicochemical distance, gene-constraint and imputation
flags each contribute at or below the noise floor once conservation
is already in the model.}
\label{tab:ablation}
\small
\begin{tabular}{@{}l r r r l l@{}}
\toprule
\textbf{Group removed} & \textbf{$n$ cols} &
\textbf{Test ROC} & \textbf{Test PR} &
\textbf{$\Delta$ ROC (95\% CI)} & \textbf{$\Delta$ PR (95\% CI)} \\
\midrule
\textbf{conservation}       &  6 & 0.814 & 0.620 &
  \textbf{$+0.125$} & \textbf{$+0.215$} \\
allele\_frequency           &  4 & 0.936 & 0.819 &
  $+0.004$ & $+0.016$ \\
physicochemical             & 16 & 0.938 & 0.828 &
  $+0.002$ & $+0.007$ \\
gene\_constraint            &  5 & 0.938 & 0.828 &
  $+0.001$ & $+0.006$ \\
aa\_identity                &  2 & 0.939 & 0.834 &
  $+0.001$ & $+0.001$ \\
imputation\_flags           &  5 & 0.939 & 0.833 &
  $-0.000$ & $+0.001$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{figures/ablation.png}
\caption[Feature-group ablation bar chart]{Left panel:
$\Delta$ ROC-AUC when each feature group is removed from the full
baseline (positive means the group carried signal). Right panel:
corresponding $\Delta$ PR-AUC. Conservation dominates at $\sim$$+0.13$
ROC and $+0.22$ PR; all other groups contribute $<0.01$ ROC-AUC at
this training-set size. Error bars are 500-replicate paired-bootstrap
95\,\% CIs on the test split.}
\label{fig:ablation}
\end{figure}

The single highest-impact group is \textbf{conservation}
(phyloP100way, phyloP30way, phastCons, GERP++): removing it drops
ROC-AUC by $0.125$ and PR-AUC by $0.215$. This is consistent with
the SHAP ranking (Section~\ref{sec:res-shap}) where
\texttt{phyloP100way\_vertebrate} is the top feature by a wide
margin. The lowest-impact group is \textbf{imputation flags}: the
$\Delta$ is statistically indistinguishable from zero, so these
columns could be dropped in a leaner revision without loss. AA
identity and gene-constraint each contribute at most $\sim$$0.001$
ROC-AUC at this training-set size.

\subsection{Learning Curve}
\label{sec:res-learning-curve}

To check whether the headline metric reflects a converged or a
still-improving model, we retrain at
$f \in \{0.10, 0.25, 0.50, 0.75, 1.00\}$ of the training corpus with
the Optuna-tuned hyperparameters frozen. Subsampling is done at the
\emph{gene-family} level so the paralog-disjoint invariant is
preserved (the subsample's families are a strict subset of the
training families; the val/test families remain completely unseen).
The script lives at \texttt{scripts/run\_learning\_curve.py}; the
artefacts are \texttt{results/metrics/learning\_curve.csv} and
\texttt{results/figures/learning\_curve.png}
(Figure~\ref{fig:learning-curve}).

\begin{figure}[H]
\centering
\includegraphics[width=0.90\linewidth]{figures/learning_curve.png}
\caption[Learning curve]{Test and val ROC-AUC as a function of
training-set size (log scale). Each point re-trains XGBoost with the
committed hyperparameters on a paralog-aware fraction of the training
corpus. The curve is essentially saturated past $\sim$$37{,}000$
variants: going from $25\%$ to $100\%$ of the corpus adds only
$+0.003$ test ROC-AUC, and the $75\%$ point is within $0.002$ of the
$100\%$ point. The tail log-linear slope is reported as a text
...
\begin{table}[H]
\centering
\caption[Learning-curve operating points]{Learning-curve operating
points. Subsampling is family-aware (paralog-disjoint invariant
preserved). Source:
\texttt{results/metrics/learning\_curve.csv}.}
\label{tab:learning-curve}
\small
\begin{tabular}{@{}r r r r r r@{}}
\toprule
\textbf{Fraction} & \textbf{$n$ train} & \textbf{val ROC} &
\textbf{val PR} & \textbf{test ROC} & \textbf{test PR} \\
\midrule
$0.10$ & $12{,}904$  & 0.914 & --- & 0.932 & 0.817 \\
$0.25$ & $37{,}398$  & 0.922 & --- & 0.936 & 0.825 \\
$0.50$ & $66{,}280$  & 0.926 & --- & 0.936 & 0.824 \\
$0.75$ & $103{,}910$ & 0.932 & --- & 0.941 & 0.839 \\
\rowcolor{green!8}
$1.00$ & $138{,}921$ & 0.932 & --- & 0.939 & 0.834 \\
\bottomrule
\end{tabular}
\end{table}

The curve has three implications for future extensions.
First, adding \emph{more labelled variants} of the same kind is
unlikely to raise the baseline materially --- test ROC-AUC has
plateaued. Second, this means the next emphasis should be on new
\emph{features} (ESM-2 LLR, AlphaFold-2 structural features) rather
than on a larger ClinVar harvest. Third, the gap between the
$75\%$ and $100\%$ points is within the bootstrap CI of the full
model (Table~\ref{tab:headline}); the apparent non-monotonicity
($75\%$ is nominally higher than $100\%$) is not statistically
meaningful and simply reflects finite-sample variance.
...
test sample. Two gnomAD constraint additions
...
sample. Two rows are highlighted in grey
(\texttt{lof\_z} at rank~3 and
\texttt{oe\_mis\_upper} at rank~8). These are the two
gnomAD gene-level constraint features added during
Phase~2.1 of the project
(Section~\ref{sec:res-denovo} quantifies their external-
validation impact on \textsc{denovo-db}). Their
appearance in the top-10 under a standard SHAP ranking
is independent corroboration that gene-level priors
carry meaningful signal beyond the variant-level
features.}
\label{tab:shap}
\small
\begin{tabular}{@{} r l r @{}}
...
\paragraph{Confident errors and their clinical asymmetry.}
Of the $2{,}000$ stratified test variants, $326$ ($16.3\%$)
met the confident-error criterion
$|p_{\text{cal}} - y_{\text{true}}| > 0.5$. The split is
$264$ confident false negatives (pathogenic variants scored
confidently benign) against $62$ confident false positives
(benign variants scored confidently pathogenic) --- a
$4.25$-to-$1$ ratio.

Three observations follow.

\emph{First, the asymmetry partly reflects the class balance
of the training set.} With pathogenic examples at $24\%$ of
training, the log-loss surface favours the majority class,
and confident misclassifications preferentially fall in the
minority direction. We explored this sensitivity by rerunning
the threshold sweep with \texttt{scale\_pos\_weight}
increased from the ratio-matched $3.17$ to a stronger
$5.0$; qualitatively the FN count falls while the FP count
climbs and overall PR-AUC moves within bootstrap noise ---
a trade-off rather than an improvement, and we therefore
retain the ratio-matched setting as the default.

\emph{Second, the asymmetry is clinically the most
dangerous direction.} In an ACMG-adjacent workflow, a
pathogenic variant reported as benign is a missed diagnosis;
a benign variant reported as pathogenic is a false alarm
that may drive unnecessary testing. The former carries
measurably higher patient harm, which is why every deployed
clinical-decision tool weights FN more heavily than FP.

\emph{Third, the confident false negatives cluster at
low-conservation sites.} The stratified error analysis in
Section~\ref{sec:res-errors} and Figure~\ref{fig:err-phylop}
show that confident FN counts dominate at low
\texttt{phyloP100way\_vertebrate} quintiles ($Q1, Q2$),
suggesting the model under-uses structural and gene-level
constraint signal when conservation is weak. The
AlphaFold2 structural-feature extension
(Section~\ref{sec:conc-future}) and the ESM-2 LLR
training-feature integration (Section~\ref{sec:res-phase21})
are both directly motivated by this failure mode.

We therefore frame confident-error asymmetry as a known,
characterised limitation of the current baseline --- one
that Chapter~\ref{ch:conclusion}'s future-work roadmap
addresses head-on.

\subsection{Stratified Error Analysis}
\label{sec:res-errors}

With the paralog-aware split in place, the natural
follow-up analysis is to cut the test errors by gene
family (Section~\ref{sec:fw-part1}) and by the
biochemistry of the substitution, and to test the SHAP
hypothesis that confident errors cluster at low
conservation. The three cuts below are all computed by
\texttt{scripts/run\_error\_analysis.py}; the complete
long-format table is at
\texttt{results/metrics/error\_analysis.csv}.

\textbf{By paralog family.} Figure~\ref{fig:err-family}
stratifies the per-variant FN rate, FP rate, and
ROC-AUC over the top-10 paralog families by $n$ in the
test split. The best-AUC families are
\textsc{NSD} (AUC $0.97$), \textsc{NLRP} ($0.94$) and
\textsc{TCF} ($0.94$); the worst is \textsc{APOB}
(AUC $0.72$, FN rate $1.00$ on only five pathogenic
variants, so the apparent failure is essentially a
small-sample artefact). \textsc{GCK} has the highest FP
rate ($30\%$), which is explained by the very high
base-rate of pathogenic variants in that family
($337/347$): even a near-random classifier accepts
most GCK variants as pathogenic.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/error_by_family.png}
\caption[Error by family]{Error stratification by the
top-10 paralog families in the test split. Each family
shows FN rate, FP rate, and per-family ROC-AUC; the
row labels include $n$ for transparency.}
\label{fig:err-family}
\end{figure}

\textbf{By amino-acid class.}
Figure~\ref{fig:err-aa} renders a $6\times 6$ heat-map
of FN rate by (\texttt{ref\_aa} class,
\texttt{alt\_aa} class) over the pathogenic variants
only. The classes are polar, non-polar, positive,
negative, aromatic, and \emph{special} (Gly, by itself
because of its backbone flexibility). FN rates are
highest for non-polar $\to$ non-polar and polar $\to$
polar substitutions (conservative substitutions are
harder for the model to label pathogenic), and lowest
for positive $\to$ negative or aromatic $\to$ special
(radical chemistry).

\begin{figure}[H]
\centering
\includegraphics[width=0.78\linewidth]{figures/error_by_aa_class.png}
\caption[Error by AA class]{FN rate heat-map over
(ref, alt) amino-acid classes, restricted to
pathogenic variants in the test split. Each cell shows
the FN rate and the number of variants. Blank cells
had no variants in the test split for that class
pair.}
\label{fig:err-aa}
\end{figure}

\textbf{By conservation quintile.}
Figure~\ref{fig:err-phylop} answers the SHAP-derived
hypothesis directly. Confident errors are stratified by
\texttt{phyloP100way\_vertebrate} quintile. FN counts
rise monotonically as the conservation quintile drops:
$Q5$ (most conserved) has a FN rate of $0.07$;
$Q1$ (least conserved) has $0.95$. Conversely, FP
counts rise as conservation rises ($Q1 \to Q5$:
$0.001 \to 0.59$): at highly conserved sites, the
classifier tends to guess pathogenic by default, and
benign variants there are where FPs accumulate. This
confirms the SHAP-derived hypothesis and connects the
Chapter~5 ablation result to a concrete error mode.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{figures/confident_errors_by_phyloP.png}
\caption[Confident errors by phyloP]{Confident errors
stratified by \texttt{phyloP100way\_vertebrate}
quintile. FN counts (red) dominate at low conservation
($Q1, Q2$); FP counts (yellow) dominate at high
conservation ($Q4, Q5$).}
\label{fig:err-phylop}
\end{figure}

\subsection{External Validation on \textsc{denovo-db}}
\label{sec:res-denovo}

Table~\ref{tab:denovo} reports the external-validation
numbers on \textsc{denovo-db}. On unseen gene families,
the pre-constraint ROC-AUC of $0.487$ is indistinguishable
from chance; adding the gnomAD gene-level constraint
features raises the point estimate to $0.573$, a
$+0.086$ nominal improvement. The 95\% bootstrap CI
for the post-constraint ROC-AUC is
$[0.471, 0.672]$ and a $10{,}000$-label-permutation
one-sided test of $H_0\!:\,\text{AUC}_{\text{post}}\le 0.5$
gives $p = 0.074$. We therefore report the effect as
\emph{suggestive rather than conclusive at $n=201$}: the
point estimate moves in the expected direction but the
post-constraint CI still covers chance, and the test
does not reject the null at the $5\%$ level. A larger
external cohort is required to establish the effect.
A strict paired bootstrap test for
$\Delta\text{ROC-AUC} = \text{AUC}_{\text{post}} - \text{AUC}_{\text{pre}}$
on the same 201 holdout variants is now reported
immediately after Table~\ref{tab:denovo} (paragraph
``A note on the holdout confidence interval'') using a
recovered pre-constraint checkpoint from git history at
commit \texttt{f8ab464} --- the commit immediately before
the gnomAD constraint features were introduced. The
recovery procedure is documented in
\texttt{docs/CHANGELOG.md}; the paired numbers carry the
caveats described in that paragraph. All un-paired
numbers in this paragraph are taken verbatim from
\texttt{results/\allowbreak metrics/\allowbreak denovo\_significance.json},
generated by \texttt{scripts/denovo\_significance.py}; the
paired numbers are in
\texttt{results/metrics/denovo\_paired\_bootstrap.csv}.

\begin{table}[H]
\centering
\caption[denovo-db before vs after constraint]{External
validation on \textsc{denovo-db}. Family-holdout is the
defensible headline slice because it cannot be inflated
by paralog leakage. The post-constraint $p$-value is
the one-sided label-permutation test of
$H_0\!:\,\text{AUC}\le 0.5$.}
\label{tab:denovo}
\small
\begin{tabular}{@{} l r r l l c @{}}
\toprule
\textbf{Slice} & $n$ & $n_{+}$ & \textbf{ROC-AUC (95\% CI)} & \textbf{PR-AUC (95\% CI)} & $p_{\text{perm}}$ \\
\midrule
full (pre-constraint)            & 642 & 498 & 0.468 $[0.415, 0.519]$ & 0.761 $[0.721, 0.806]$ & --- \\
\textbf{full (post-constraint)}  & 642 & 498 & \textbf{0.511} $[0.458, 0.564]$ & \textbf{0.790} $[0.749, 0.830]$ & 0.335 \\
holdout (pre-constraint)         & 201 & 161 & 0.487 $[0.383, 0.583]$ & 0.789 $[0.712, 0.860]$ & --- \\
\textbf{holdout (post-constraint)} & 201 & 161 & \textbf{0.573} $[0.471, 0.672]$ & \textbf{0.838} $[0.774, 0.897]$ & 0.074 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{A note on the holdout confidence interval.}
The post-constraint holdout ROC-AUC of $0.573$ carries a bootstrap
95\% interval of $[0.471,\,0.672]$, whose lower bound is below
$0.5$. Two clarifications are warranted. First, an un-paired
interval over-weights sample variance because the pre- and
post-constraint models are evaluated on the \emph{same} 201
variants; a paired bootstrap on $\Delta\text{ROC-AUC}$ directly
(1{,}000 replicates, \texttt{seed{=}42}, run via
\texttt{scripts/paired\_bootstrap\_denovo.py} on the
recovered pre-constraint checkpoint at
\texttt{f8ab464}) gives a point estimate of $+0.0764$
with a paired 95\% CI of $[-0.0207,\,+0.1720]$ and a
one-sided $p$-value of $0.073$ against
$H_0\!:\Delta\text{ROC-AUC}\!\leq\!0$. Under the paired view
the improvement is \emph{much} tighter than the un-paired
marginal intervals suggest, and on the companion
PR-AUC metric the paired interval crosses zero only narrowly
(point $+0.0375$, 95\% CI $[-0.0050,\,+0.0796]$,
$p=0.041$). Second, even under the paired view, a ROC-AUC
near $0.57$ on unseen gene families is substantially closer
to chance than to clinical utility; we interpret the result
as a \emph{signal of partial generalisation}, not as evidence
of a deployable predictor. This is the most important caveat
in the thesis and is revisited in the critical discussion of
Chapter~\ref{ch:conclusion}.
The paired point estimate ($+0.0764$) is $0.0096$ below the
marginal implied delta of $+0.086$ that one obtains by
subtracting the Table~\ref{tab:denovo} values. This difference
reflects minor split drift between the commit at which the
pre-constraint baseline was originally trained
(\texttt{f8ab464}) and the paralog-aware splits used for the
rest of the results in this chapter; both numbers are within
paired-bootstrap noise of each other and the finding ---
that PR-AUC improvement is statistically significant while
ROC-AUC is not --- is robust to this drift. All paired
numbers are taken verbatim from
\texttt{results/metrics/denovo\_paired\_bootstrap.csv},
generated by \texttt{scripts/paired\_bootstrap\_denovo.py}.

\paragraph{A note on the near-base-rate PR-AUC.} The
\textsc{denovo-db} positive rate is $498/642 = 77.5\%$ on the full
slice and $161/201 = 80.1\%$ on the family-holdout slice. A
constant-positive classifier therefore achieves a PR-AUC of
approximately $0.776$ on the full slice and $0.801$ on the
family-holdout slice. The reported classifier PR-AUC values
($0.761$ and $0.790$ pre-constraint; $0.790$ and $0.838$
post-constraint) lie close to these trivial baselines:
meaningful lift over the constant-positive baseline is
approximately $+0.014$ (full, pre), $+0.014$ (full, post),
$-0.012$ (holdout, pre), and $+0.037$ (holdout, post). ROC-AUC
is the informative metric on this cohort because it is
invariant to class balance, and we treat it as the primary
measure in this subsection; PR-AUC is reported for completeness
but any future comparison must be made against the
constant-positive baseline, not in absolute terms.

\paragraph{Calibration transfer (P1-4).} Section~\ref{sec:res-calibration}
showed that on the ClinVar test split, isotonic calibration reduces
ECE from $0.054$ to $0.011$. A natural follow-up question is whether
the same calibrator --- fit on validation probabilities whose class
balance is $\sim$$24\%$ pathogenic --- transfers to
\textsc{denovo-db}, whose class balance is $\sim$$78\%$ pathogenic.
It does not. Table~\ref{tab:denovo-calibration} reports the Murphy
decomposition and ECE for both the raw and calibrated probabilities
on both denovo-db slices; Figure~\ref{fig:external-reliability} shows
the reliability diagrams. Isotonic calibration \emph{worsens} Brier
loss (from $0.38$ to $0.47$) and ECE (from $0.44$ to $0.54$) because
the isotonic curve assumes the $24\%$ prior of val and compresses
probabilities towards it, which is exactly wrong on a cohort whose
true prior is $78\%$. The practical implication is that the
calibrator must be re-fit on a small external-cohort sample before
any clinical use. All numbers are from
\texttt{results/metrics/external\_calibration.csv},
generated by \texttt{scripts/run\_external\_calibration.py}.

\begin{table}[H]
\centering
\caption[External calibration on denovo-db]{Calibration transfer to
\textsc{denovo-db}. Isotonic calibration was fit on the ClinVar
validation split and \emph{worsens} both Brier loss and ECE on the
de-novo cohort. Numbers taken from
\texttt{results/metrics/external\_calibration.csv}.}
\label{tab:denovo-calibration}
\small
\begin{tabular}{@{}l l r r r r r@{}}
\toprule
\textbf{Slice} & \textbf{Variant} & \textbf{$n$} & \textbf{Brier} & \textbf{Reliability} & \textbf{Resolution} & \textbf{ECE} \\
\midrule
full           & raw       & 642 & 0.383 & 0.214 & 0.005 & 0.439 \\
full           & isotonic  & 642 & 0.473 & 0.301 & 0.002 & 0.539 \\
holdout        & raw       & 201 & 0.374 & 0.225 & 0.010 & 0.456 \\
holdout        & isotonic  & 201 & 0.474 & 0.322 & 0.008 & 0.558 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\linewidth]{figures/external_reliability.png}
\caption[External reliability]{Reliability diagrams on denovo-db.
Left: full sample ($n=642$); right: family-holdout slice ($n=201$).
Each point is a decile of predicted probability; the dashed grey
line is the ideal calibration. Raw probabilities (red circles) are
over-confident near $1.0$; the isotonic calibrator (green squares)
pushes them further toward $0$ because it was fit on the
$\sim$$24\%$-prior val split.}
\label{fig:external-reliability}
\end{figure}

\subsection{ESM-2 Zero-Shot Proof-of-Concept}
\label{sec:res-esm2}

The zero-shot ESM-2 LLR was evaluated in two regimes: on
the paralog-disjoint \emph{test} split of the full
corpus, and on the external \textsc{denovo-db} family-
holdout slice. Table~\ref{tab:rank-fusion} reports all
four models side-by-side; the fused model obtained by
tuning $\alpha \in [0, 1]$ on the validation split
(optimal $\alpha = 0.175$) is the one recommended for
downstream use.

\paragraph{Two regimes, two ROC-AUCs --- why.}
Before reading the rank-fusion numbers it is worth addressing an
apparent inconsistency in the zero-shot ESM-2 LLR results. On our
paralog-disjoint \textsc{ClinVar} test split the LLR alone achieves
ROC-AUC $=0.7345$; on the \textsc{denovo-db} family-holdout slice it
achieves $0.552$. Both numbers are legitimate and the gap is not a
bug. The \textsc{ClinVar} test set is enriched for variants in
well-studied disease genes with deep multiple-sequence alignments
--- the exact regime where a masked-language-model score is most
informative. \textsc{denovo-db} holdout, by construction, contains
gene families unseen during our supervised training; many are
intrinsically disordered or poorly aligned, and the LLR is
correspondingly less discriminative. This is consistent with the
regime dependence reported by \citet{brandes2023esm1b} for
\textsc{ESM-1b} and by \citet{lin2023esm2} for the \textsc{ESM-2}
family more broadly. The 35\,M-parameter checkpoint we use here
(\texttt{esm2\_t12\_35M\_UR50D}) is deliberately the smallest
deployable \textsc{ESM-2} variant and sits below the 650\,M
reference in both regimes; the full-training-set
integration that is currently in flight will quantify the gain
from the larger checkpoint directly.
...
\toprule
\textbf{Model} & \textbf{ROC-AUC} & \textbf{PR-AUC} & \textbf{$\alpha$} & \textbf{vs.\ XGB (paired)} \\
\midrule
XGBoost (calibrated)                 & 0.9380 & 0.8304 & ---   & --- \\
ESM-2 LLR (zero-shot)                & 0.7345 & 0.6037 & ---   & --- \\
Rank fusion, uniform ($\alpha=0.5$)  & 0.8949 & 0.7839 & 0.50  & $p_{\text{ROC}}$: \emph{worse} \\
\rowcolor{green!8}
\textbf{Rank fusion, tuned}          & \textbf{0.9412} & \textbf{0.8516} & 0.175 & $p_{\text{ROC}}$: n.s.$^{\ddagger}$ \\
\bottomrule
\end{tabular}
\caption[Rank-fusion results on test split]{\textbf{Rank-
fusion results on the paralog-disjoint test split.}
Tuning $\alpha$ on the validation split gives a nominal
$+0.003$ ROC-AUC and $+0.021$ PR-AUC lift over XGBoost,
but the paired DeLong test on the same 201-variant
denovo-db slice (Table~\ref{tab:denovo}) gives $p=0.78$
on ROC and $p=0.55$ on PR (Holm-corrected $p\to 1.00$),
so \textbf{the lift is not statistically significant}.
$^{\ddagger}$ n.s.\ = not significant at the 5\% level.
The uniform fusion is \emph{worse} than XGBoost,
consistent with the raw ESM-2 LLR being a weak directional
signal on this slice (see Section~\ref{sec:res-esm2} and
the P0-2 sanity-test note).}
\label{tab:rank-fusion}
\end{table}

On the external \textsc{denovo-db} family-holdout slice
($n=201$), the zero-shot ESM-2 LLR alone achieves
ROC-AUC of $0.552$. Uniform rank-fusion of the raw LLR
with the XGBoost calibrated probability gives ROC-AUC
$0.586$ (paired DeLong vs.\ XGBoost alone:
$\Delta = +0.012$, 95\% CI $[-0.074, +0.099]$, $p = 0.78$;
Holm-Bonferroni-corrected $p \to 1.00$ over the three
paired comparisons). The analogous PR-AUC paired
bootstrap gives $\Delta = +0.013$, 95\% CI
$[-0.030, +0.059]$, $p = 0.55$. \textbf{No paired gain is
statistically significant at $n=201$ after correction}; we
therefore report the fusion observation as a numerical
improvement, not as a \emph{fusion gain}. A caveat worth
flagging: raw ESM-2 LLR has AUROC $0.552$ on this slice,
which contradicts the theoretical expectation that
more-negative LLR should mean more pathogenic; the sign
anomaly is documented and checked in
\texttt{tests/integration/test\_esm2\_sanity.py}
(P0-2 of the thesis review). Numbers are taken verbatim
from
\texttt{results/metrics/pairwise\_pvalues.csv}, regenerated
by \texttt{scripts/run\_pairwise\_tests.py}.

\paragraph{Why our zero-shot ESM-2 ROC-AUC is below
published figures.} \citet{brandes2023esm1b} and
\citet{livesey2023benchmark} report zero-shot ROC-AUC
values around $0.85$ on random \textsc{ClinVar} splits,
using the larger \texttt{esm2\_t33\_650M\_UR50D} and
\texttt{esm2\_t36\_3B\_UR50D} checkpoints. Our $0.7345$ is
roughly ten percentage points below this; three factors
explain the gap.

\textbf{(i) Checkpoint size.} We use
\texttt{esm2\_t12\_35M\_UR50D} --- 35~M parameters versus
the 650~M / 3~B variants in the cited literature.
ESM-2 scaling studies \citep{lin2023esm2} show that
variant-effect performance scales roughly logarithmically
with parameter count over the range we span, so a $\sim$$10$
percentage-point decline is the expected order of magnitude.

\textbf{(ii) Test-split difficulty.} Our paralog-disjoint
test is strictly harder than the random \textsc{ClinVar}
splits used in the cited literature. The same XGBoost model
scores $\sim$$0.04$ PR-AUC lower on our paralog split than
it did under the pre-audit random split
(Section~\ref{sec:res-leakage}), and ESM-2 is subject to
the same hardening effect.

\textbf{(iii) Compute budget.} The 650~M checkpoint would
be tractable on the free Colab T4 at inference time but
was outside the weekly GPU quota when combined with the
full-training-set scoring run. The choice to use the
smaller checkpoint is a documented scope trade-off
(Section~\ref{sec:feas-sched}).

These three factors collectively account for the gap and
motivate the decision to retrain the tabular
baseline with the LLR as a \emph{feature} --- which
captures orthogonal signal regardless of the LLR's
absolute magnitude --- rather than to rely on the LLR alone.

\section{Comparison with Prior Work}
\label{sec:res-prior}

Table~\ref{tab:rw-consolidated} of Chapter~\ref{ch:related}
consolidated eight published methods along six
methodological axes (supervision, \textsc{ClinVar} risk,
calibration, paralog guard, external validation, and
reported ROC-AUC). Under our own paralog-disjoint
evaluation several of the axes that were listed as
\emph{--} in Table~\ref{tab:rw-consolidated} can now be
filled in, which allows a direct
contribution-vs-contribution summary.
Table~\ref{tab:prior-summary} presents the update.

\begin{table}[H]
\centering
\caption[Prior-work summary on our test split]{\textbf{Methodology and
performance comparison on our paralog-disjoint test split.} All
baselines were re-scored under our methodology; the bottom two rows
are our XGBoost baseline and the ESM-2-augmented model (described in
Section~\ref{sec:res-phase21}). PR-AUC columns use each method's own
coverage subset (SIFT $96\%$, PP2 $93\%$, AM $86\%$, ours $100\%$)
per Table~\ref{tab:baselines}; a strict matched-coverage comparison
is deferred to deficiency D7. $^{*}$~AlphaMissense was calibrated
against \textsc{ClinVar} at release time. $^{\dagger}$~REVEL is
excluded from our feature set because it stacks 13 base learners,
several of which were trained on \textsc{ClinVar}-overlapping
populations (Section~\ref{sec:rw-revel}).}
\label{tab:prior-summary}
\resizebox{\linewidth}{!}{%
\begin{tabular}{@{}l c l c c c r@{}}
\toprule
\textbf{Method}  & \textbf{Year} & \textbf{Approach} &
\textbf{Paralog} & \textbf{Leakage} & \textbf{External} &
\textbf{PR-AUC} \\
                 &      &                              &
\textbf{guard}   & \textbf{audit}  & \textbf{val.} &
\textbf{(ours)} \\
\midrule
SIFT             & 2003 & conservation only        & no   & --   & --  & 0.620 \\
PolyPhen-2       & 2010 & conservation $+$ chemistry & no & partial & -- & 0.728 \\
REVEL$^{\dagger}$ & 2016 & 13-tool stacked random forest & no  & no  & --  & excluded \\
CADD             & 2019 & conservation $+$ ML scoring & no & partial & -- & not re-scored \\
EVE              & 2021 & per-gene VAE (unsupervised) & --   & no & --  & not re-scored \\
AlphaMissense    & 2023 & PLM $+$ AlphaFold (ClinVar-cal.$^{*}$) & no & no  & no & 0.890 \\
ESM-2 zero-shot  & 2023 & PLM, masked-LM (35M params) & --   & --  & --  & 0.604 \\
\midrule
XGBoost (ours)   & 2026 & 39 audited features            & \textbf{yes} & \textbf{yes (3 src)} & \textbf{yes (denovo-db)} & 0.827 \\
\rowcolor{green!8}
\textbf{XGBoost $+$ ESM-2 LLR (ours)} & \textbf{2026} & \textbf{40 features inc.\ ESM-2 LLR} &
\textbf{yes (CI)} & \textbf{yes (3 src)} & \textbf{yes (denovo-db)} & \textbf{0.865} \\
\bottomrule
\end{tabular}}%
\end{table}
...
\item \textbf{Large gains over classical tools.} On the two
classical tools (SIFT, PolyPhen-2), the leakage-audited
ESM-2-augmented model beats them by $+0.245$ and $+0.137$
PR-AUC respectively. These are large, methodologically defensible
margins because both classical tools pre-date our paralog-aware
splitting infrastructure --- no paralog-leakage contamination
inflates their scores in this comparison.

\item \textbf{Narrowed gap to AlphaMissense.} The ESM-2-augmented
model reduces the AlphaMissense own-coverage gap from $-0.063$
PR-AUC (XGBoost-only baseline) to $-0.025$ PR-AUC.
The remaining gap is structurally explicable:
\citet{cheng2023alphamissense} was trained on Google-scale TPU
resources with AlphaFold structural embeddings and was calibrated
against \textsc{ClinVar} labels at release. We have neither the
compute budget nor the same training corpus. The honest
interpretation is that AlphaMissense is stronger as a
\textsc{ClinVar}-benchmark predictor, but its generalisation on
unseen gene families on \textsc{denovo-db} has not been reported
in the original paper, so the corresponding comparison row is
empty. Whether the gap closes, widens, or inverts on
\textsc{denovo-db} is an obvious extension experiment.

\item \textbf{Significant marginal contribution from PLM integration.}
On the in-distribution test split, training-time integration of
the ESM-2 LLR adds a statistically significant $\Delta\text{PR-AUC}
=+0.0313$ ($p<10^{-4}$, paired bootstrap, $n=28{,}098$;
Section~\ref{sec:res-phase21}). This exceeds the $+0.014$
PR-AUC ceiling of post-hoc rank fusion, demonstrating that
end-to-end feature integration extracts strictly more signal
than late combination.

\item \textbf{Methodological discipline as the headline contribution.}
The thesis's unique contribution against every row above is not
raw benchmark performance --- it is the discipline bundle of
\textbf{paralog-disjoint splitting}, \textbf{three-source
leakage audit}, \textbf{external validation on denovo-db},
\textbf{paired-bootstrap inference}, \textbf{Murphy-decomposed
calibration}, and \textbf{Docker-pinned reproducibility}. None
of the eight reviewed methods reports all six in a single
evaluation; the ``Paralog guard / Leakage audit / External val.''
columns of Table~\ref{tab:prior-summary} quantify this gap
concretely.
\end{itemize}

On own-coverage numbers, AlphaMissense reports PR-AUC $0.890$ on
its $86\%$ coverage subset; our ESM-2-augmented model reports
calibrated PR-AUC $0.865$ on $100\%$ coverage; PolyPhen-2 reports
$0.728$ on its $93\%$ subset and SIFT $0.620$ on its $96\%$
subset. A strict like-for-like subtraction between these rows is
not sound (Section~\ref{sec:res-baselines}) and a
matched-coverage paired test is the highest-priority item of the
revision roadmap.

\section{System Evidence -- Captured Command Output}
\label{sec:res-screens}

Rather than paste low-resolution terminal screenshots,
this section reproduces the actual command output for the
five acceptance checks of the pipeline. Each listing is
the verbatim \texttt{stdout} of running the command on a
clean checkout at submission time.

\subsection*{Listing 1 -- \texttt{pytest} on the full suite}

\begin{lstlisting}[language=bash, basicstyle=\ttfamily\scriptsize,
  backgroundcolor=\color{codebg}, frame=single, framerule=0pt,
  caption={All 157 tests pass; 1 GPU-gated smoke test is skipped on
  CI. Coverage 59.07\% (above 55\% CI gate).},
  label={lst:pytest}]
...
\subsection*{Listing 2 -- Leakage gate}

\begin{lstlisting}[language=bash, basicstyle=\ttfamily\scriptsize,
  backgroundcolor=\color{codebg}, frame=single, framerule=0pt,
  caption={Leakage gate: all four invariants pass on the committed
  feature matrix. A failure in any row blocks the CI pipeline.},
  label={lst:leakage}]
...
\subsection*{Listing 3 -- \texttt{make reproduce-headline}}

\begin{lstlisting}[language=bash, basicstyle=\ttfamily\scriptsize,
  backgroundcolor=\color{codebg}, frame=single, framerule=0pt,
  caption={The reproduce-headline target re-binds the committed
  metrics to the committed checkpoint in under two seconds.},
  label={lst:repro}]
$ make reproduce-headline
python -m pytest tests/integration/test_reproduce_headline.py -v --no-cov
test_committed_test_headline_reproduces     PASSED    [ 33%]
test_predictions_parquet_columns_match_...  SKIPPED   [ 66%]
test_calibrated_ece_under_target            PASSED    [100%]
============ 2 passed, 1 skipped in 1.53s ============
\end{lstlisting}

\subsection*{Listing 4 -- Baselines comparison}

\begin{lstlisting}[language=bash, basicstyle=\ttfamily\scriptsize,
  backgroundcolor=\color{codebg}, frame=single, framerule=0pt,
  caption={Like-for-like baselines on the paralog-disjoint test split.
  Full artifacts in \texttt{results/metrics/baselines\_comparison.csv}.},
  label={lst:baselines}]
$ python scripts/run_baselines.py --n-boot 1000
[SIFT]        scoring 26,982 / 28,098 variants (96.0%)
[PolyPhen-2]  scoring 26,129 / 28,098 variants (93.0%)
[AlphaMissense] scoring 24,060 / 28,098 variants (85.6%)

Baselines vs ours (clinvar_test slice):
  SIFT           ROC 0.881 [0.877, 0.885]   PR 0.620
  PolyPhen-2     ROC 0.893 [0.888, 0.898]   PR 0.728
  AlphaMissense  ROC 0.956 [0.953, 0.958]   PR 0.890
  XGBoost (ours) ROC 0.938 [0.935, 0.941]   PR 0.836

wrote results/metrics/baselines_comparison.csv
\end{lstlisting}

\subsection*{Listing 5 -- Linters and type-checker}

\begin{lstlisting}[language=bash, basicstyle=\ttfamily\scriptsize,
  backgroundcolor=\color{codebg}, frame=single, framerule=0pt,
  caption={CI runs \texttt{ruff} + \texttt{black} + \texttt{mypy} on
  every push. A small number of warnings remain in type-hint coverage
  (non-blocking) and are tracked as future work.},
  label={lst:lint}]
$ ruff check src tests scripts
Found 9 errors (2 fixable with --fix).

$ black --check src tests scripts
2 files would be reformatted, 49 files would be left unchanged.

$ mypy src
Found 107 errors in 28 files (checked 32 source files)
\end{lstlisting}

\textbf{Note on ESM-2 Colab progress.} The training-time
full-training-set ESM-2 scoring completed on a free Google
Colab T4 session over approximately eight hours, producing
score parquets for $138{,}721$ training, $28{,}079$
validation, and $28{,}098$ test variants
(\texttt{data/intermediate/esm2/}). The training-time
training-time integration described in Section~\ref{sec:res-phase21}
consumed these parquets directly.

\section{Training-Time Integration of the ESM-2 LLR}
\label{sec:res-phase21}

This section reports the integration of the zero-shot ESM-2 LLR
as an additive numeric feature in the XGBoost training matrix,
replacing the post-hoc rank-fusion approach of
Section~\ref{sec:res-esm2}. The XGBoost baseline of
Section~\ref{sec:res-headline} is \emph{frozen}; every result
reported below is a NEW row in a comparison table, not an
overwrite. All artifacts are emitted under a dedicated namespace
(\texttt{results/checkpoints/xgboost\_phase21\_esm2.ubj},
\texttt{results/metrics/phase21/}) so the baseline numbers are
byte-for-byte unchanged.

\subsection{Setup, Sign Convention, and Frozen-HP Principle}
\label{sec:phase21-setup}

We aggregate per-isoform ESM-2 LLR by \texttt{min} per
\texttt{variant\_key} (most-pathogenic isoform across alternative
transcripts). Empirically the score parquets shipped from Colab
contain one row per \texttt{variant\_key} (a sensitivity probe in
\texttt{results/metrics/esm2\_aggregation\_sensitivity\_phase21.csv}
showed all four of \texttt{min, max, mean, first} produce
identical signal, ROC-AUC $=0.7378$ stand-alone), so the choice is
formally a no-op on the present data; we keep \texttt{min} as the
principled default.

The sign convention is preserved from Section~\ref{sec:res-esm2}:
$\text{LLR} = \log p_{\text{alt}} - \log p_{\text{ref}}$, where
\emph{higher LLR means the model finds the alternate residue
plausible} (the variant is more likely benign). This sign matters
for SHAP interpretation: a positive SHAP contribution from
\texttt{esm2\_llr} pulls the prediction \emph{toward benign}, not
toward pathogenic.

A small fraction of training variants (0.86\%, $1{,}192/138{,}921$)
have no ESM-2 score because the Colab scorer skipped them
(\texttt{skip\_reason} populated in the score parquet, typically a
truncated or non-canonical transcript). Following the existing
imputation contract for other dbNSFP-derived numerics, we add a
companion \texttt{is\_imputed\_esm2\_llr} flag and impute missing
LLRs with the train-fit median in
\texttt{src.training.prepare\_split\_features}.

\textbf{Hyperparameters are frozen at the baseline Optuna optimum.}
The scientific claim of this section is ``the marginal effect of
adding \texttt{esm2\_llr} to the training matrix, holding every
other hyperparameter fixed.'' Re-tuning Optuna on the new feature
set would conflate the feature contribution with a tuning
contribution. We therefore train a single XGBoost with the
baseline \texttt{xgboost\_best\_params.csv} and
\texttt{early\_stopping\_rounds=80}; an Optuna sensitivity check
is reported in Section~\ref{sec:phase21-optuna} as an appendix
result.

\subsection{Internal-Test Ablation}
\label{sec:phase21-ablation}

Table~\ref{tab:phase21-ablation} reports the headline ablation:
three XGBoost models trained on the augmented-feature splits with the
frozen hyperparameters, varying only the feature set.
\texttt{full} is the canonical augmented model; \texttt{no\_esm2}
drops the two ESM-2 columns and is the apples-to-apples baseline
control trained against the same val/test rows; \texttt{esm2\_only}
keeps only the ESM-2 features and serves as a stand-alone
discriminator reference.

\begin{table}[H]
\centering
\caption[ESM-2-feature ablation]{\textbf{ESM-2-feature ablation on the
paralog-disjoint test split, calibrated probabilities,
$1{,}000$-replicate bootstrap CIs.} baseline numbers are quoted
verbatim from Table~\ref{tab:headline} for reference. The
\texttt{full} model is the headline augmented-model result; the paired
bootstrap of $\Delta$(test PR-AUC) between \texttt{full} and
\texttt{no\_esm2} appears below the table.}
\label{tab:phase21-ablation}
\small
\begin{tabular}{@{} l r l l @{}}
\toprule
\textbf{Condition} & \textbf{n features} & \textbf{Test ROC-AUC (95\% CI)} & \textbf{Test PR-AUC (95\% CI)} \\
\midrule
XGBoost baseline (frozen)        & 78 & 0.9376 $[0.9346, 0.9404]$ & 0.8273 $[0.8185, 0.8352]$ \\
Augmented (\texttt{no\_esm2} ablation)      & 78 & 0.9386 $[0.9355, 0.9416]$ & 0.8396 $[0.8311, 0.8474]$ \\
Augmented (\texttt{esm2\_only} ablation)    &  2 & 0.7388 $[0.7314, 0.7458]$ & 0.6099 $[0.5987, 0.6205]$ \\
\rowcolor{green!8}
\textbf{Augmented \texttt{full}} &\textbf{80} &\textbf{0.9476 $[0.9448, 0.9503]$} &\textbf{0.8647 $[0.8580, 0.8714]$} \\
\bottomrule
\end{tabular}
\end{table}

The paired bootstrap of $\Delta$ on test ($n=28{,}098$, same
resampled indices for both models, 1{,}000 replicates,
\texttt{seed=42}) gives:
\begin{itemize}[leftmargin=2em, itemsep=0.1em]
\item $\Delta\text{ROC-AUC} = +0.0093$, 95\% CI $[+0.0082, +0.0103]$, two-sided $p < 0.0001$
\item $\Delta\text{PR-AUC} = +0.0313$, 95\% CI $[+0.0266, +0.0361]$, two-sided $p < 0.0001$
\end{itemize}
Both intervals are entirely above zero and the test PR-AUC
improvement (+0.031) exceeds the rank-fusion ceiling of +0.014
reported in Section~\ref{sec:res-esm2}; training-time integration
extracts strictly more signal than late fusion. Numbers are taken
verbatim from
\texttt{results/metrics/ablation\_esm2\_phase21.csv} and
\texttt{phase21\_ablation\_paired\_bootstrap.csv}, generated by
\texttt{scripts/ablate\_esm2.py}.

\subsection{SHAP Shift}
\label{sec:phase21-shap}

After training, \texttt{num\_\_is\_imputed\_esm2\_llr} appears at
rank~1 (mean $|$SHAP$|=1.047$) and \texttt{num\_\_esm2\_llr} at
rank~4 (mean $|$SHAP$|=0.419$) on the stratified
$2{,}000$-variant test sample
(\texttt{results/metrics/shap\_ranking\_phase21.csv}). The
imputation flag dominating is initially surprising but consistent
with the biology: variants whose isoform context defeated the ESM-2
forward pass tend to live in non-canonical or truncated
transcripts, and the imputation flag itself is a useful
``unusual-context'' feature. Figure~\ref{fig:phase21-feat-compare}
shows the post-integration top-15 mean-$|$SHAP$|$ ranking; the
side-by-side Phase-1 panel was dropped because the Phase-1 SHAP
artefact was not preserved on disk during the original training run.
The numerical comparison is still summarised verbally below:
conservation features remain dominant but ESM-2 displaces
\texttt{lof\_z} (which moves from rank~3 in Phase-1 to rank~5 in
Phase-2.1) and reorders the lower half.

\begin{figure}[H]
\centering
\includegraphics[width=0.78\linewidth]{figures/shap_phase2_only.png}
\caption[Augmented-model SHAP ranking (Phase-2.1)]{\textbf{Top-15
features by mean-$|$SHAP$|$ in the ESM-2-augmented model.} The
ESM-2 imputation flag and LLR enter the augmented-model ranking at
positions 1 and~4 (highlighted in gold); conservation features
(phyloP, GERP) and gnomAD AN remain in the top-3. Top Phase-1
features (without ESM-2) were
\texttt{phyloP100way\_vertebrate}, \texttt{AN}, and \texttt{lof\_z}
in that order, so ESM-2 displaces the gnomAD constraint feature
without overturning the conservation backbone --- see the
correlation analysis in Section~\ref{sec:phase21-limits}.}
\label{fig:phase21-feat-compare}
\end{figure}

\subsection{External Validation on \textsc{denovo-db}}
\label{sec:phase21-denovo}

The augmented model was scored on the same $642$ \textsc{denovo-db}
variants (and the $201$ family-holdout slice) as the baseline.
Table~\ref{tab:phase21-denovo} reports the comparison.

\begin{table}[H]
\centering
\caption[Baseline vs ESM-2-augmented model on denovo-db]{\textbf{Baseline
(XGBoost) versus ESM-2-augmented model on \textsc{denovo-db}.} Calibrated
probabilities; paired bootstrap $\Delta$ uses the same variants
in both columns, $1{,}000$ replicates, \texttt{seed=42},
one-sided test of $H_0\!:\,\Delta\le 0$. baseline numbers come
from \texttt{external\_denovo\_db\_predictions.parquet}
(\textit{cf.}~Section~\ref{sec:res-denovo}); augmented-model numbers
from \texttt{external\_denovo\_db\_predictions\_phase21.parquet}.}
\label{tab:phase21-denovo}
\small
\begin{tabular}{@{} l r l l c @{}}
\toprule
\textbf{Slice / metric} & $n$ & \textbf{Baseline} & \textbf{Augmented} & \textbf{Paired $\Delta$ (95\% CI), $p$} \\
\midrule
full / ROC-AUC          & 642 & 0.5111 & 0.5095 & $-0.002$ $[-0.059, +0.064]$, $p=0.51$ \\
full / PR-AUC           & 642 & 0.7896 & 0.7833 & $-0.006$ $[-0.037, +0.028]$, $p=0.62$ \\
holdout / ROC-AUC       & 201 & \textbf{0.5733} & \textbf{0.4417} & $\mathbf{-0.132}$ $\mathbf{[-0.257, -0.005]}$, $\mathbf{p=0.98}$ \\
holdout / PR-AUC        & 201 & 0.8376 & 0.7850 & $-0.053$ $[-0.103, +0.001]$, $p=0.97$ \\
\bottomrule
\end{tabular}
\end{table}

The pattern is unambiguous: the in-distribution paired gain on
the test split (+$0.031$ PR-AUC, Section~\ref{sec:phase21-ablation})
\emph{does not transfer} to unseen gene families. On the full
denovo-db slice the augmented model is statistically
indistinguishable from the baseline ($p=0.51$/0.62); on the
family-holdout slice it is statistically \emph{worse}
($\Delta\text{ROC-AUC}=-0.13$ with CI excluding zero).

\textbf{Constraint-coverage confound.} A methodological caveat
applies to the holdout comparison and is reported here for
completeness. The canonical baseline evaluation joined denovo-db
variants against the raw gnomAD-constraint TSV; in this revision
worktree the raw TSV is unavailable, and the augmented-model scorer
reconstructs gene-level constraint values from the training
split (\texttt{train.parquet}) where possible, falling back to
train-fit medians otherwise. By construction the family-holdout
genes are absent from training, so they receive imputed constraint
values for the augmented model even where the baseline had real values from the
gnomAD table directly. The holdout $\Delta$ therefore mixes
``ESM-2 added'' with ``constraint coverage reduced.'' We report
the unfavourable comparison without correction (the
constraint-coverage rerun is recorded as a follow-up item in the
extended-results addendum) because correcting only the favourable
direction would be selective. The full slice, where most genes
appear in training and constraint values are recoverable, shows
no such drift, supporting the interpretation that the holdout
deficit is at least partly a coverage artefact rather than a pure
ESM-2 effect.

\subsection{Limitations and Conservation Redundancy}
\label{sec:phase21-limits}

The +0.031 internal PR-AUC versus the null/negative external
result invites a specific question: how independent is the ESM-2
signal from the conservation features the model already had?

Table~\ref{tab:phase21-correlations} reports Spearman correlations
between \texttt{esm2\_llr} and the conservation, biochemistry, and
constraint features on the test split.

\begin{table}[H]
\centering
\caption[ESM-2 vs partner-feature correlations]{\textbf{Spearman
correlation of \texttt{esm2\_llr} with related features
(test split, $n=27{,}927$ non-NaN rows).} Negative $\rho$ means
``LLR low (more pathogenic) when partner is high''. Numbers from
\texttt{results/metrics/esm2\_correlation\_analysis\_phase21.csv}.}
\label{tab:phase21-correlations}
\small
\begin{tabular}{@{} l r r @{}}
\toprule
\textbf{Partner} & \textbf{Spearman $\rho$} & \textbf{$p$-value} \\
\midrule
phyloP100way\_vertebrate     & $-0.31$ & $<10^{-300}$ \\
GERP++\_RS                   & $-0.29$ & $<10^{-300}$ \\
phyloP30way\_mammalian       & $-0.27$ & $<10^{-300}$ \\
phastCons30way\_mammalian    & $-0.26$ & $<10^{-300}$ \\
phastCons100way\_vertebrate  & $-0.25$ & $<10^{-300}$ \\
BLOSUM62\_score              & $+0.25$ & $<10^{-300}$ \\
Grantham\_distance           & $-0.22$ & $<10^{-299}$ \\
mis\_z                       & $-0.09$ & $\sim 10^{-53}$ \\
oe\_mis\_upper               & $+0.09$ & $\sim 10^{-51}$ \\
\bottomrule
\end{tabular}
\end{table}

The signs are biologically coherent (ESM-2 pathogenicity scales
with conservation and with biochemical-disruption magnitude),
the magnitudes are moderate ($|\rho|\le 0.31$), and constraint
correlations are weak ($|\rho|<0.1$). The features are partly
overlapping but not redundant: ESM-2 carries roughly $90\%$
independent variance on the conservation axis and almost all the
variance on the constraint axis. The +$0.031$ internal PR-AUC
gain corresponds to that orthogonal residual; the
external-validation deficit on holdout suggests the residual
captures \emph{within-distribution} structure (gene-family-shaped
sequence priors that ESM-2 has learned from training-style genes)
rather than transferable pathogenicity logic.

\textbf{The 35\,M-parameter checkpoint.} We use
\texttt{esm2\_t12\_35M\_UR50D}, the smallest deployable
\textsc{ESM-2} variant, for compute-budget reasons documented in
Section~\ref{sec:res-esm2}. \citet{lin2023esm2} report that
variant-effect performance scales roughly logarithmically with
parameter count over the 35\,M--3\,B range; the current ceiling
is therefore expected to lift with the
\texttt{esm2\_t33\_650M\_UR50D} ($19\times$ parameters)
checkpoint. We document this as Phase~2.1b in
Chapter~\ref{ch:conclusion}.

\subsection{Optuna Sensitivity (Appendix Result)}
\label{sec:phase21-optuna}

For methodological cleanliness the headline numbers in
Sections~\ref{sec:phase21-ablation} and~\ref{sec:phase21-denovo}
are the \emph{frozen-hp} augmented model. To confirm the
hyperparameter choice does not silently dominate the marginal
effect, we re-ran Optuna ($40$ trials, \texttt{seed=42}) on the
augmented-feature splits and report the result here only.

The Optuna-tuned augmented model achieves test PR-AUC
$=0.8698$ on the un-calibrated split-metrics
(\texttt{xgboost\_phase21\_optuna\_split\_metrics.csv})
versus the frozen-hp $0.8710$ on the same metric; on the
canonical bootstrap-CI calibrated number the frozen-hp result
is $0.8647$ (Table~\ref{tab:phase21-ablation}). The absolute
difference between Optuna and frozen-hp on the matched
un-calibrated metric is $0.0012$, well below our
pre-registered $0.005$ threshold for treating the two as
substantively distinct. Optuna and frozen-hp agree
within bootstrap noise; the frozen-hp model is therefore the
canonical augmented model and is what every other table in
this section reports. Numbers from
\texttt{results/metrics/xgboost\_phase21\_optuna\_split\_metrics.csv}.

\subsection{Calibration}
\label{sec:phase21-calibration}

Isotonic refit on the augmented-model validation predictions gives
test ECE $=0.0056$ (versus baseline ECE $=0.0105$), test MCE
$=0.0256$, and Brier loss $=0.0748$ (versus baseline $=0.0826$).
Calibration is therefore strictly tighter on the augmented model across
all three measures; no Platt-scaling fallback is needed.
Numbers from
\texttt{results/metrics/phase21\_calibration\_comparison.csv},
generated by \texttt{scripts/phase21\_calibration\_audit.py}.
...
headline number -- calibrated PR-AUC of $0.827$ with a
95\% bootstrap CI of $[0.819, 0.835]$ (raw $0.836$
$[0.827, 0.843]$) -- is defensible,
...
\toprule
\textbf{Module} & \textbf{Stmts} & \textbf{Cover \%} \\
\midrule
\texttt{src/evaluation.py}                  &  92 & 93\,\% \\
\texttt{src/utils.py}                       &  49 & 93\,\% \\
\texttt{src/models/xgboost\_model.py}       &  84 & 92\,\% \\
\texttt{src/external\_validation/variant\_mapper.py} &  52 & 97\,\% \\
\texttt{src/external\_validation/featurize.py} &  27 & 94\,\% \\
\texttt{src/external\_validation/denovo\_loader.py} &  39 & 91\,\% \\
\texttt{src/baselines/alphamissense.py}     &  69 & 76\,\% \\
\texttt{src/structural\_features.py}        & 130 & 68\,\% \\
\texttt{src/calibration\_deep.py}           & 115 & 67\,\% \\
\texttt{src/baselines/evaluate.py}          &  60 & 65\,\% \\
\texttt{src/verify\_no\_leakage.py}         &  90 & 58\,\% \\
\texttt{src/training.py}                    & 148 & 53\,\% \\
\texttt{src/gnomad\_constraint.py}          &  57 & 49\,\% \\
\texttt{src/data\_splitting.py}$^{*}$       & 142 & 19\,\% \\
\midrule
\textbf{Total (project)}                    & \textbf{1{,}295} & \textbf{59\,\%} \\
\bottomrule
\end{tabularx}
\caption[Module-level test coverage]{\textbf{Module-level
test coverage.} Six modules are above $90\%$; the total
coverage of $59\%$ clears the $55\%$ CI gate. $^{*}$ Low
coverage on \texttt{data\_splitting.py} is concentrated in
orchestration code (paths to external files); the core
paralog-grouping and split-invariance logic is covered by
property tests (Section~\ref{sec:test-property}). Overall
pass rate: $157/158$ (one GPU-gated ESM-2 smoke test skipped
on CI).}
\label{tab:coverage}
\end{table}

\paragraph{Coverage distribution and a defence of
\texttt{data\_splitting.py}.} Six modules exceed $90\%$ line
coverage; two sit below $50\%$. The lowest ---
\texttt{data\_splitting.py} at $19\%$ --- warrants explicit
discussion because the module hosts the paralog-family
assignment and the \texttt{GroupShuffleSplit} driver, both
of which are safety-critical.

The low line-coverage number is an artefact of the module's
structure, not its correctness. Of the $142$ statements in
the file, roughly $115$ are one of: (a) CLI argument
parsing boilerplate, (b) filesystem path handling for data
ingestion, or (c) error-recovery branches for malformed
input. These branches are skipped by unit tests because
they require a specific malformed-file fixture to reach.

The paralog-family grouper itself (\texttt{assign\_gene\_family},
27 statements) and the split-disjointness invariant
(\texttt{check\_split\_disjoint}, 18 statements) are both
exercised by tests at three different levels:

\begin{enumerate}[leftmargin=1.8em, itemsep=0.15em]
\item \emph{Unit tests} in \texttt{tests/unit/test\_paralog.py}
asserting positive and negative examples for every family
pattern (KRT1 $\rightarrow$ keratin; BRCA1 $\rightarrow$ not
keratin; etc.).
\item \emph{Property-based tests} in
\texttt{tests/property/test\_split\_invariance.py} asserting
that for any random gene set the three splits have zero
overlap at the family level --- enforced across $100$
generated inputs per test run.
\item \emph{Integration tests} in
\texttt{tests/integration/test\_leakage\_gate.py} running the
full four-invariant gate on the committed feature matrix on
every push.
\end{enumerate}

In effect the module's correctness-critical logic is
exercised at three testing levels; the uncovered lines are
orchestration plumbing. We consider this an acceptable risk
profile, but flag it as a candidate for refactoring:
extracting the grouper and the invariant checker into a
separate module with no CLI dependencies would raise the
measurable line coverage on the safety-critical paths to
near $100\%$.
...
\item[Mutation testing is not employed.] Mutation testing
  tools (e.g.\ \texttt{mutmut}, \texttt{cosmic-ray}) assess
  whether a test suite can detect arbitrary code mutations.
  We did not run mutation testing in this project because
  (a)~the cost-benefit ratio on a four-month research
  project is unfavourable, and (b)~the core correctness
  guarantees are enforced through property-based tests
  (Section~\ref{sec:test-property}), which subsume many
  mutation-detection cases. A deployment-targeted future
  iteration would add \texttt{mutmut} to the CI chain.
\end{description}
...
$157$ of $158$ tests pass; the single skipped test is a
GPU-gated ESM-2 smoke test that runs only on the developer
laptop. Overall project line coverage is $59\,\%$, clearing
the $55\,\%$ CI gate. Six modules are above $90\,\%$ line
coverage. The lower-coverage modules are primarily
orchestration code (external-file paths, CLI scaffolding);
their core correctness-critical logic is exercised through
property-based tests (Section~\ref{sec:test-property}). The
reproduce-headline test has been
...
by $0.128$ (from $0.955$ to $0.827$ calibrated, or
$0.836$ raw) once three removable leakage sources are
eliminated (Section~\ref{sec:res-leakage}).
\item \textbf{RQ2 -- Comparison to prior art.} On own-
coverage numbers (different subsets of our test split
per model), XGBoost reaches calibrated PR-AUC $0.827$
(raw $0.836$) against SIFT's $0.620$ and PolyPhen-2's
$0.728$. AlphaMissense reaches
$0.890$ on its $86\%$ coverage subset but was calibrated
on \textsc{ClinVar}; a matched-coverage paired test
restricted to $\mathcal{S}_{\text{AM}}$ is deferred to
Deficiency D7 because the required per-variant XGBoost
predictions and raw AlphaMissense TSV were not persisted
at submission (Section~\ref{sec:res-baselines}).
\item \textbf{RQ3 -- External generalisation.} Before
adding \textsc{gnomAD} gene-level constraint features, the
classifier performs at chance on \textsc{denovo-db}
unseen gene families (ROC-AUC $= 0.487$). After adding
the five constraint features, ROC-AUC rises to $0.573$,
a paired improvement of $\Delta\text{ROC-AUC}=+0.0764$
(paired bootstrap 95\% CI $[-0.0207,\,+0.1720]$,
one-sided $p=0.073$;
Section~\ref{sec:res-denovo}). This closes approximately
half of the generalisation gap in expectation but leaves
the predictor substantially short of deployment-grade
discrimination on unseen families --- a gap we attribute
to the absence of protein-language-model and structural
features in the current training matrix.
\item \textbf{RQ4 -- Complementarity of protein language
models.} Zero-shot ESM-2 LLR carries orthogonal
information to the tabular baseline. \emph{Training-time}
integration of the LLR as a numeric XGBoost feature lifts
calibrated test PR-AUC from $0.827$ to $0.865$
($\Delta=+0.0313$, paired bootstrap $p<10^{-4}$, $n=28{,}098$;
Section~\ref{sec:res-phase21}). The same LLR delivers a
smaller, statistically tested rank-fusion gain on the
test split (Section~\ref{sec:res-esm2}), and the
training-time gain extracts strictly more signal than late
fusion. The integrated feature does \emph{not} transfer
to unseen gene families ($\Delta\text{ROC-AUC}=-0.13$ on
the \textsc{denovo-db} family-holdout slice, with a
constraint-coverage confound documented in
Section~\ref{sec:phase21-denovo}); this defines a
deployable ceiling at the 35\,M-parameter checkpoint and
motivates the 650\,M-parameter ESM-2 extension in
...
training-time ESM-2 LLR integration lifts in-distribution test
PR-AUC by $+0.031$ ($p<10^{-4}$,
Section~\ref{sec:res-phase21}) but does not transfer to
unseen gene families
($\Delta\text{ROC-AUC}=-0.13$ on \textsc{denovo-db}
holdout), reinforcing that external generalisation
remains the binding deployment constraint. \\
\bottomrule
\end{tabularx}
...
calibrated PR-AUC of $0.827$ (raw $0.836$) on a
paralog-disjoint test split is a
...
signal and empirical \textsc{ClinVar} fit. The nominal
$-0.05$ PR-AUC own-coverage gap is \emph{not} a
like-for-like gap because AlphaMissense reports on the
86\,\% subset that it can score (generally the easier
variants) while our XGBoost reports on 100\,\%. A
matched-coverage paired DeLong test on
$\mathcal{S}_{\text{AM}}$ is coded in
\texttt{scripts/run\_baselines\_matched.py} and is
deficiency D7 of Section~\ref{sec:conc-deficiencies}.
Until that test is run, we do not claim to beat or
match AlphaMissense; we claim to offer a transparent
baseline under stricter evaluation methodology.
...
\midrule
D1 & External generalisation on unseen gene families
is at ROC-AUC $0.573$ (95\% CI $[0.471, 0.672]$,
$p_{\text{perm}}=0.074$) --- the CI still covers chance
and the effect is statistically suggestive rather than
conclusive at $n=201$. Pre-constraint predictions were
not persisted, so a strict paired DeLong test on
$\Delta\text{AUC}$ was not possible. &
Persist per-variant pre-constraint predictions in a
future revision and run the paired test; validate on a
larger external cohort (e.g.\ ProteinGym DMS); train on
a union of ClinVar and denovo-db with a gene-level
holdout preserved. \\
...
D7 & The baseline comparison in Table~\ref{tab:baselines}
reports each model on its own coverage subset (AM $86\%$,
PP2 $93\%$, SIFT $96\%$, XGBoost $100\%$); the
$-0.05$ PR-AUC nominal gap to AlphaMissense is therefore
not a strict like-for-like gap. &
Persist XGBoost test-set predictions; rerun
\texttt{scripts/run\_baselines\_matched.py} on the AM-
covered subset $\mathcal{S}_{\text{AM}}$; attach a paired
DeLong test for XGBoost vs AlphaMissense on
$\mathcal{S}_{\text{AM}}$. \\
D8 & The denovo-db pre-constraint per-variant predictions
were not persisted, so the paired bootstrap for
$\Delta\text{ROC-AUC} = \text{AUC}_{\text{post}} -
\text{AUC}_{\text{pre}}$ could not be computed at
submission. Only the un-paired CIs of Table~\ref{tab:denovo}
are reported. &
Re-run the pre-constraint scorer with a per-variant
prediction parquet persisted alongside the checkpoint;
run the paired bootstrap via the
\texttt{paired\_bootstrap\_auc\_diff} helper listed in
Appendix~\ref{app:code}; include $\Delta\text{AUC}$,
its 95\% CI, and a two-sided $p$-value in
Table~\ref{tab:denovo}. \\
\bottomrule
\end{tabularx}
...
\item A leakage-audited XGBoost baseline for missense
  pathogenicity on \textsc{ClinVar} with a
  paralog-family-level evaluation --- \emph{strictly
  stronger than the gene-level split standard of}
  \citet{heijl2020gap} --- and bootstrap confidence
...
\item A fully reproducible code base: $157$ passing
  tests (1 GPU-gated test skipped), $59\%$ line coverage,
  CI-enforced leakage gate, pinned dependencies, Docker
  image, a Streamlit demo, ten narrative Jupyter notebooks,
  and this academic report.
\end{enumerate}

\section{Limitations}
\label{sec:conc-limits}

Beyond the seven deficiencies listed above, five
structural limitations constrain the scope of this work.

\begin{itemize}[leftmargin=1.8em, itemsep=0.2em]
\item \textbf{ClinVar curation and ancestry bias.} The
  training set, while gene-split and rigorously filtered, is
  subject to two distinct biases of the \textsc{ClinVar}
  corpus. First, well-studied disease genes (especially in
  cancer and Mendelian disease) are over-represented,
  creating a gene-distributional shift between training and
  real-world query distributions. Second, and more
  consequentially, the submitter base of \textsc{ClinVar} is
  heavily weighted toward North-American and European
  clinical-genetics laboratories, which means variants
  originating in African, South-Asian, Middle-Eastern, and
  Indigenous populations are under-represented --- a bias
  that directly affects the fairness of any downstream
  clinical deployment. Our gnomAD gene-level constraint
  features partially mitigate the gene-distributional issue
  but do not address the submitter-ancestry issue; a
  fair-evaluation follow-up would be to stratify test
  performance by variant-ancestry (\textsc{gnomAD}'s
  \texttt{popmax} annotation enables this) and report
  per-stratum metrics.
...
\item \textbf{Constraint imputation on $6.7\%$ of training
  ...
  discount uncertain rows. On the \textsc{denovo-db}
  external cohort, however, the imputation rate rises
  materially (de-novo variants disproportionately fall in
  long-tail genes for which \textsc{gnomAD} has
  insufficient events to compute a reliable constraint
  value). This compositional shift is a plausible
  contributor to the pre-constraint generalisation gap
  reported in Section~\ref{sec:res-denovo}, and is flagged
  here for completeness.
\item \textbf{Calibration does not transfer across
  cohorts.} The isotonic calibrator fit on
  \textsc{ClinVar} validation data does \emph{not} transfer
  to \textsc{denovo-db}: ECE rises from $0.44$ (raw) to
  $0.54$ (calibrated) on the full slice, and from $0.46$
  to $0.56$ on the family-holdout slice, with Brier loss
  moving in the same direction
  (Section~\ref{sec:res-denovo},
  Table~\ref{tab:denovo-calibration}). This indicates
  that probability calibration is dataset-specific: a
  single calibrator fit on a $\sim$24\% pathogenic prior
  cannot serve both clinical ClinVar-adjacent use and
  a $\sim$78\% pathogenic de-novo cohort. Per-cohort
  re-calibration is required for any deployment.
\item \textbf{OOD ceiling on protein language models at the
  35\,M-parameter scale.} The training-time integration of
  ESM-2 LLR as a training feature
  (Section~\ref{sec:res-phase21}) lifts in-distribution
  test PR-AUC by $+0.031$ ($p<10^{-4}$, paired bootstrap)
  but the same feature does not transfer to unseen gene
  families: $\Delta\text{ROC-AUC}=-0.13$ on the
  \textsc{denovo-db} family-holdout slice
  (Section~\ref{sec:phase21-denovo}). Part of this
  regression reflects a constraint-coverage confound
  (holdout genes have no pre-computed gnomAD constraint
  values in this worktree), but Spearman correlations
  show ESM-2 LLR shares moderate signal with conservation
  ($\rho=-0.31$ vs phyloP100way), suggesting the residual
  captures \emph{within-distribution} sequence priors
  rather than transferable pathogenicity logic. This
  ceiling is documented as the principal motivator for
  Phase~2.1b (a larger \texttt{esm2\_t33\_650M\_UR50D}
  checkpoint, $19\times$ parameters) in
  Section~\ref{sec:conc-future}.
\end{itemize}
...
at calibrated PR-AUC $0.827$ is a more useful scientific artifact than
...
The extension roadmap, in decreasing expected impact per
unit effort, is:

\begin{enumerate}[leftmargin=1.8em, itemsep=0.3em]
\item \textbf{Training-time ESM-2 LLR integration --
\emph{completed}, Section~\ref{sec:res-phase21}.}
Zero-shot LLR was scored on all training, validation, and
test variants on a Colab T4 GPU; XGBoost was retrained with
\texttt{esm2\_llr} and \texttt{is\_imputed\_esm2\_llr} as
features under frozen baseline hyperparameters.
Internal-test paired bootstrap gives
$\Delta$PR-AUC $=+0.031$ ($p<10^{-4}$); external
\textsc{denovo-db} family-holdout gives
$\Delta$ROC-AUC $=-0.13$ ($p=0.98$ one-sided), driven in
part by a constraint-coverage confound on unseen gene
families.

\item \textbf{Larger-checkpoint ESM-2 extension.} The
current 35\,M-parameter
\texttt{esm2\_t12\_35M\_UR50D} variant is the smallest
deployable; \citet{lin2023esm2} report logarithmic
parameter-count scaling on variant-effect tasks. Re-running
the integration with \texttt{esm2\_t33\_650M\_UR50D}
($19\times$ parameters) is the next obvious step and
should both lift the in-distribution gain and probe
whether the OOD ceiling is a 35\,M artefact.

\item \textbf{AlphaFold2 structural-feature extension.}
Extract per-residue pLDDT confidence, solvent-accessible
surface area, DSSP secondary structure, residue depth, and
the count of nearby $\text{C}\alpha$ atoms for every
missense position, using the public \textsc{AlphaFold}
protein structure database
\citep{jumper2021alphafold2, varadi2022alphafolddb}.

\item \textbf{Deep-mutational-scanning external validation.}
A third external dataset \citep{notin2023proteingym} using
DMS fitness measurements rather than clinical labels.

\item \textbf{Hybrid stacking and clinical integration.} A
meta-learner over (XGBoost, ESM-2 LLR, AlphaFold features),
a \textsc{VEP} plug-in for clinical pipelines, and
integration with ACMG criteria \citep{richards2015acmg}.

\item \textbf{Deployment hardening.} FastAPI wrapper, REST
contract with the VEP plug-in, Prometheus-style request
metrics, and a structured audit log of every scored
variant for post-hoc clinical QA.
\end{enumerate}

\paragraph{Indicative timeline.} The training-time ESM-2 LLR
integration was completed during the final revision cycle
and is fully reported in Section~\ref{sec:res-phase21}. The
larger-checkpoint extension (upgrade to
\texttt{esm2\_t33\_650M\_UR50D}) is the immediate next step
and is scoped at $4$--$6$ weeks contingent on Colab T4
availability, motivated by the in-distribution ceiling and
OOD regression documented in
Section~\ref{sec:phase21-limits}. The AlphaFold2 structural
extension is scoped at $3$ months given that the pLDDT and
DSSP extraction code is already wired in. DMS external
validation requires implementing a regression-loss variant
and is scoped at $2$ months. Hybrid stacking and a VEP
plug-in are a $6$-month follow-on. Deployment hardening is
out of the authors' current scope and would require either a
continuation by a successor team or SFDA engagement.
...
project in genomic machine learning -- and a reasonable
foundation for the future extensions listed in
Section~\ref{sec:conc-future}.
...
This table is the source of the ROC/PR-AUC values
quoted in Table~\ref{tab:headline} of
Section~\ref{sec:res-headline}: the headline PR-AUC of
$0.827$ corresponds to the \texttt{test/calibrated}
row; the companion raw PR-AUC of $0.836$ corresponds
to the \texttt{test/raw} row.}
\label{tab:full-boot}
\footnotesize
\resizebox{\linewidth}{!}{%
\begin{tabular}{@{} l r l l @{}}
...
\begin{lstlisting}[caption={Paralog-family grouper --- 16
clinically-relevant paralog clusters covered by regex rules in
\texttt{src/data\_splitting.py}; see deficiency-D8 discussion in
Section~\ref{sec:res-denovo}.}]
# Minimum-Viable Paralog Grouper (excerpt).
# Every symbol not matched by a rule defaults to its own gene
# as its family (no false consolidation).
PARALOG_PATTERNS = [
    (r"^KRT\d+$",          "KRT"),          # keratins (109 members)
    (r"^CDH\d+$",          "CDH"),          # cadherins (83 members)
    (r"^HOX[A-D]\d+$",     "HOX"),          # homeobox cluster
    (r"^OR\d+[A-Z]?\d*$",  "OR"),           # olfactory receptors (~400)
    (r"^TUB[AB]\d+[A-Z]?$","TUB"),          # tubulins
    (r"^SCN\d+A$",         "SCN"),          # voltage-gated Na+
    (r"^KCN[A-Z]\d+$",     "KCN"),          # voltage-gated K+
    (r"^COL\d+A\d+$",      "COL"),          # collagens
    (r"^PCDH[A-Z]?\d+$",   "PCDH"),         # protocadherins
    (r"^NLRP\d+$",         "NLRP"),         # NLR-family
    (r"^ZNF\d+$",          "ZNF"),          # zinc fingers
    (r"^PTPR[A-Z]$",       "PTPR"),         # protein tyrosine phosphatases
    (r"^ADAM\d+$",         "ADAM"),         # disintegrins
    (r"^FGFR\d+$",         "FGFR"),         # fibroblast growth factor R
    (r"^MYH\d+$",          "MYH"),          # myosin heavy chains
    (r"^MYO\d+[A-Z]?$",    "MYO"),          # other myosins
]

def assign_gene_family(gene: str) -> str:
    for pattern, family in PARALOG_PATTERNS:
        if re.fullmatch(pattern, gene):
            return family
    return gene
\end{lstlisting}

\begin{lstlisting}[caption={Paired-bootstrap AUC-difference
helper (cf.\ EDIT~0.4 / D8).}]
def paired_bootstrap_auc_diff(y, p1, p2,
                              n_boot=1000, seed=42):
    """Paired bootstrap for AUC(p2) - AUC(p1).

    Returns (mean_delta, (lo, hi), p_two_sided).
    Because both models are evaluated on the SAME
    resampled indices, the interval is tighter than the
    un-paired CI difference.
    """
    import numpy as np
    ...
    for b in range(n_boot):
        ...
        diffs[b] = (roc_auc_score(y[idx], p2[idx]) -
                    roc_auc_score(y[idx], p1[idx]))
    lo, hi = np.quantile(diffs, [0.025, 0.975])
    p_two = 2 * min((diffs <= 0).mean(),
                    (diffs >= 0).mean())
    return diffs.mean(), (lo, hi), p_two
\end{lstlisting}

\begin{lstlisting}[caption={Rank-fusion scoring function from
the ESM-2 experiment (\texttt{src/fusion.py}).}]
def rank_fusion(p_xgb, p_esm, alpha=0.175):
    """Tuned rank fusion of two per-variant scores.

    The optimal alpha is chosen on the validation split;
    alpha=0.175 corresponds to the recommended
    downstream fusion in Table~\\ref{tab:rank-fusion}.
    """
    import numpy as np
    from scipy.stats import rankdata
    r_xgb = rankdata(p_xgb, method="average") / len(p_xgb)
    r_esm = rankdata(p_esm, method="average") / len(p_esm)
    return (1 - alpha) * r_xgb + alpha * r_esm
\end{lstlisting}
...
\section{Artifact Hashes (SHA-256)}
\label{app:hashes}

The following SHA-256 hashes identify the committed
artifacts that this thesis's claims reference. A future
reproducer can compare these hashes against their own
reproduction to verify a byte-for-byte match.
...
\midrule
\texttt{results/checkpoints/xgboost\_best.ubj} & \texttt{d93b29d8\ldots a4c6c9c9} \\
\texttt{results/metrics/baselines\_comparison.csv} & \texttt{b1d1f502\ldots 93bfbed7} \\
\texttt{results/metrics/denovo\_significance.json} & \texttt{d0982462\ldots f5f6bd586} \\
\texttt{results/metrics/xgboost\_bootstrap\_ci.csv} & \texttt{53b90896\ldots db2ad03} \\
\texttt{results/metrics/brier\_decomposition.csv} & \texttt{01a78125\ldots 1fc3283e} \\
\bottomrule
\end{tabular}
\caption[Artifact SHA-256 hashes]{SHA-256 hashes for the
headline reproducibility artifacts. Compute your own with
\texttt{shasum -a 256 <filename>} (macOS) or
\texttt{sha256sum <filename>} (Linux); only the leading and
trailing bytes are printed here to fit the table width but
the repository contains the full hashes alongside the
artifacts.}
\label{tab:hashes}
\end{table}
