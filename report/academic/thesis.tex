...
A strict paired DeLong/bootstrap test for
$\text{AUC}_{\text{post}} - \text{AUC}_{\text{pre}}$ was
not possible because the pre-constraint predictions
originate from a separately-trained model whose
per-variant scores were not persisted; this is recorded
as a deficiency in Section~\ref{sec:conc-deficiencies}
and would be the first item on any revision. All
numbers in this paragraph are taken verbatim from
\texttt{results/\allowbreak metrics/\allowbreak denovo\_significance.json},
generated by \texttt{scripts/denovo\_significance.py}.
...
\paragraph{A note on paired vs un-paired intervals.} Because the
pre- and post-constraint models are evaluated on the \emph{same}
$201$ family-holdout variants, an un-paired comparison of the two
bootstrap CIs over-weights sample variance: the intervals
$[0.383, 0.583]$ and $[0.471, 0.672]$ overlap substantially, which
would na\"ively suggest no significant improvement. A \emph{paired}
bootstrap --- resampling the same 201 indices for both models and
computing $\Delta\text{ROC-AUC} = \text{AUC}_{\text{post}}(y[\text{idx}],
p_{\text{post}}[\text{idx}]) - \text{AUC}_{\text{pre}}(y[\text{idx}],
p_{\text{pre}}[\text{idx}])$ per replicate --- is the correct
instrument and is implemented in
\texttt{scripts/denovo\_significance.py} via the
\texttt{paired\_bootstrap\_auc\_diff} helper reproduced in
Appendix~\ref{app:code}. At submission time the paired test could
not be run because the per-variant pre-constraint predictions were
not persisted during the first training run; this is recorded as
deficiency D8 in Section~\ref{sec:conc-deficiencies} and is the
single highest-priority revision item. The un-paired interpretation
in the paragraph above is therefore conservative, not liberal: any
sharpening of the paired interval would \emph{strengthen} rather
than weaken the ``suggestive'' verdict.

\paragraph{A note on the holdout confidence interval.} The
post-constraint holdout ROC-AUC of $0.573$ carries a bootstrap
95\% interval of $[0.471, 0.672]$, whose \emph{lower bound is below
$0.5$}. This is the single most important caveat in the thesis and
we do not hide it. Even under the paired view, a ROC-AUC near $0.57$
on unseen gene families is substantially closer to chance than to
clinical utility; we interpret the result as a \textbf{signal of
partial generalisation} --- the right direction of motion from the
chance-level pre-constraint point estimate --- \textbf{not} as
evidence of a deployable predictor. This caveat is revisited in the
critical discussion of Section~\ref{sec:conc-critical}, and the
direct implication for RQ3 is stated in
Section~\ref{sec:conc-main}.
...
the point estimate moves to $0.573$ \emph{after}. The
95\% bootstrap CI for the post-constraint AUC is
$[0.471, 0.672]$ and still covers $0.5$; a one-sided
label-permutation test gives $p=0.074$. We therefore
report the effect as suggestive rather than conclusive
at $n=201$ and flag it as the first candidate for a
larger external validation cohort
