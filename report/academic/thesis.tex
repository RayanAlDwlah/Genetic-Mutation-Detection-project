...
\centering
\footnotesize
\caption[Prior-work summary on our test split]{Summary of
published methods against the thesis baseline on our
paralog-disjoint test split. PR-AUC columns use each
method's own coverage subset (SIFT $96\%$, PP2 $93\%$, AM
$86\%$, ours $100\%$) per Table~\ref{tab:baselines}; a
strict matched-coverage comparison is deferred to
deficiency D7.}
\label{tab:prior-summary}
\begin{tabular}{@{}l l l l l r@{}}
\toprule
\textbf{Method} & \textbf{Supervision} & \textbf{Paralog guard} & \textbf{External val.} & \textbf{ClinVar risk} & \textbf{PR-AUC (ours)} \\
\midrule
SIFT           & unsup.              & no   & --                         & none    & 0.620 \\
PolyPhen-2     & sup.                & no   & --                         & partial & 0.728 \\
AlphaMissense  & sup.                & no   & --                         & high    & 0.890 \\
\rowcolor{green!8}
\textbf{This work} & \textbf{sup.\ + PLM fusion} & \textbf{yes (CI-enforced)} & \textbf{yes (denovo-db)} & \textbf{audited} & \textbf{0.827} \\
\bottomrule
\end{tabular}
\end{table}
...
the leakage-audited baseline beats them by $+0.21$ and
$+0.10$ PR-AUC respectively. These are large,
methodologically defensible margins --- because SIFT and
PolyPhen-2 pre-date our paralog-splitting infrastructure,
no paralog-leakage contamination inflates their score.

\emph{Second,} against AlphaMissense, the $-0.063$ PR-AUC
gap (calibrated) is meaningful but structurally explicable.
\citet{cheng2023alphamissense} was trained on Google-scale
TPU resources with AlphaFold structural embeddings and was
calibrated against \textsc{ClinVar} labels at release. We
have neither the compute budget nor the same training
corpus. The honest interpretation is: AlphaMissense is
stronger as a \textsc{ClinVar}-benchmark predictor, but its
generalisation on unseen gene families on
\textsc{denovo-db} has not been reported in the original
paper, so the corresponding comparison row is empty.
Whether the $-0.063$ \textsc{ClinVar} gap closes, widens,
or inverts on \textsc{denovo-db} is an open question and an
obvious Phase-2 experiment.

\emph{Third,} the thesis's unique contribution against
every row of Table~\ref{tab:rw-consolidated} is not raw
benchmark performance --- it is the methodological
discipline bundle of paralog-disjoint splitting, leakage
audit, external validation, calibration reporting, and
reproducibility enforcement. None of the eight reviewed
methods reports all five in a single evaluation.

On own-coverage numbers,
AlphaMissense reports PR-AUC $0.890$ on its $86\%$
coverage subset, our XGBoost reports calibrated
$0.827$ (raw $0.836$) on $100\%$;
PolyPhen-2 reports $0.728$ on its $93\%$ subset and SIFT
$0.620$ on its $96\%$ subset. A strict like-for-like
subtraction between these rows is not sound
(Section~\ref{sec:res-baselines}) and a matched-coverage
paired test is the highest-priority item of the revision
roadmap.
