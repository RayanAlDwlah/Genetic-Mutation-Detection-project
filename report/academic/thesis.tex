...
Rank fusion, uniform ($\alpha=0.5$) & 0.8949 & 0.7839 & 0.50 \\
\rowcolor{green!8}
\textbf{Rank fusion, tuned}    & \textbf{0.9412} & \textbf{0.8516} & 0.175 \\
\bottomrule
\end{tabular}
\caption[Rank-fusion results on test split]{\textbf{Rank-
fusion results on the paralog-disjoint test split.}
Tuning $\alpha$ on the validation split gives a
$+0.003$ ROC-AUC and $+0.021$ PR-AUC lift over XGBoost
alone, while zero-shot ESM-2 alone is well below
XGBoost. The uniform fusion ($\alpha=0.5$) is
\emph{worse} than XGBoost -- evidence that the naive
average over-weights the weaker PLM signal.}
