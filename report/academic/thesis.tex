...
shows the side-by-side top-15 mean-$|$SHAP$|$ ranking before vs
after the integration; conservation features remain dominant but
ESM-2 displaces \texttt{lof\_z} (which moves from rank~3 to
rank~5) and reorders the lower half.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/phase21_feature_importance_comparison.png}
\caption[Baseline vs augmented-model SHAP comparison]{\textbf{Top-15
features by mean-$|$SHAP$|$, baseline vs ESM-2-augmented model.} The
ESM-2 imputation flag and LLR enter the augmented-model ranking at
positions 1 and~4; conservation features (phyloP, GERP) and
gnomAD AN remain in the top-3. The reshuffling of the lower
half is consistent with ESM-2 partly substituting for some
conservation signal --- see the correlation analysis in
Section~\ref{sec:phase21-limits}.}
